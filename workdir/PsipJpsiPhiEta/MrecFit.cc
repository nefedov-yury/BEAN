...
//    int ngw = gw.size();
//    cout << " DEBUG ngw= " << ngw << " wsum= " << wsum-gw[0] << endl;
...
//             double bgscl = p[4] + xm*(p[5]+xm*p[6]); // p[4-6]
            double bgscl = PolN(xm, &p[4], npol); // p[4,4+npol]
...
//    fitter.Config().ParSettings(3).SetLimits(0.0,1.0);

//    for ( int i = 0; i < 4; ++i ) {      // fix parameters of signal
//    for ( int i = 4; i < 5+Npol; ++i ) { // fix parameters of background
...
//   fitter.CalculateHessErrors();
  fitter.CalculateMinosErrors();
   ...
   rescale_bg( hst[13], &par[4], Npol );
   hst[14]=(TH1D*)hst[4]->Clone("RescaledBGn");
   rescale_bg( hst[14], &par[4], Npol );
...
//    TLegend* leg = new TLegend(0.55,0.67,0.89,0.89);
