...
//    TH1D* hst = Subtract(fname); // subtract side-band
   TH1D* hst1 = plot_Mkk(fname,string("mkk_data_cp"),0);
   TH1D* hst2 = plot_Mkk(fname,string("mkk_data_sb"),1);
   TH1D* hst = (TH1D*)hst1->Clone("sub");
   hst -> Add(hst1,hst2,1.,-1.);
   if( date == 2009 ) {
      hst->SetMinimum(-10.);
   } else {
      hst->SetMinimum(-10.);
   ...
   TF1* fun = new TF1("fun", Lintfr, bL, dU, 6);
   fun->SetParNames("Mphi","#sigma", "A", "N#phi","F_{bkg}","#vartheta");
   if ( date == 2009 ) {
      fun->SetParameters(Mphi,1.5e-3, 0.98,  850., 0.007, 0.);
   } else { // 2012
//       fun->SetParameters(Mphi,1.0e-3, 0.98, 2600., 0.03, -1.0);
      fun->SetParameters(Mphi,1.0e-3, 0.98, 2600., 0.03, +1.0);
   }
...
//    gStyle->SetFitFormat(".3g");
   ...
   fun->FixParameter(2, 0.98);   // A
   fun->SetParLimits(4, 0., 1.); // Fbkg
   ...
   hst->Draw("EP");
   hst->Fit("fun","EV","",dL,dU);
//    TFitResultPtr res = hst->Fit("fun","SEV","",dL,dU);
//    res->Print();
   ...
   TLegend* leg = new TLegend(0.62,0.37,0.98,0.57);
   leg->AddEntry(hst,title.c_str(),"LEP");
...
//       fun->SetParameters(1.0e-3, 0.97, 2250.,  0.04,       0,  0);
      fun->SetParameters(1.2e-3, 0.98, 2600.,  1.e-3,  -M_PI,  0);
   }

   fun->FixParameter(5, 0); // MUST BE FIXED!

   fun->FixParameter(1, 0.98);  // A
   fun->SetParLimits(3, 0., 1.); // Fbkg
...
//    gChi2->SetTitle(";#vartheta, degrees;#chi^{2} / ndf");
   gChi2->SetTitle(";#vartheta, degrees;#chi^{2}");
...
//-------------------------------------------------------------------------
   TH1D* hst1 = Subtract("data_12psip_all.root"); // subtract side-band
   TH1D* hst2 = Subtract("data_09psip_all.root"); // subtract side-band
   hst1->SetMinimum(-25.);
   hst2->SetMinimum(-10.);
...
// ------------- data: interference ---------------
//    test_Intfr();
   string pdf = string("mkk")+((id==0) ? "09" : "12")+"_ch2_p.pdf";
