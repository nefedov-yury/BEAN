...
// ---------------------------------------------
//
// static const int Nbins = 101; // for data_fit && data_Infr_fit()!
// static const int Nbins = 110; // standard
// static const int Nbins = 121; // uncertainties study: 99, 121
//
...
#include "cuts.h"
   ...
   title += string(";Entries/") + string(Form("%.1fMeV/c^{2}",bW*1e3));
...
//       else { // DEBUG
//          sub->SetBinError(ib,fabs(sub->GetBinContent(ib)));
//       }
...
//       double err2 = SQ(hst->GetBinError(ib)) +
//                     SQ(1e-2*hst->GetBinContent(ib));
//       hst-> SetBinError(ib,sqrt(err2));
//    }
...
//----------------------------------------------------------------------
// Argus function with left cutoff
//----------------------------------------------------------------------

//----------------------------------------------------------------------
double RevArgus(double m, double A, double B, double L, double U) {
//----------------------------------------------------------------------
//  reversed ARGUS function: (arXiv:1805.05613v3) p.7:
//
//      Fbkg(m) = m·(1−(X−m)^2/t^2)^a * exp(−b·(1−(X−m)^2 /t^2 )) (4)
//
//  where X is the sum of the lower and upper limits of the fit (U+L)
//  range, a and b are constant coefficients, and t is the upper
//  limit of the fit range (U).
//
//  L,U - must be const for fit
   ...
   double tmp = 1 + (L-m)/U; // == (X-m)/t
   tmp = 1-tmp*tmp;

   return m*exp(A*log(tmp) - B*tmp);
}

//----------------------------------------------------------------------
double RevArgusN(double m, double A, double B, double L, double U) {
//----------------------------------------------------------------------
// Numeric normalisation on one for range [dL,dU]
   ...
   auto Lint = [](double x, void* pp) -> double{
      double* p = static_cast<double*>(pp);
      return RevArgus(x,p[0],p[1],p[2],p[3]);
   };
   ...
   static double cacheB = 0;
   if ( cacheN > 0 && A == cacheA && B == cacheB ) {
      norm = cacheN;
   } else {
      // desired errors:                abs    rel
   ...
   return RevArgus(m,A,B,L,U) / norm;
}
...
//-------------------------------------------------------------------------
   ...
   auto Lan = [](const double* x,const double* p) -> double {
      return p[0]*RevArgusN(x[0],p[1],p[2],dL,dU);
   };

   TF1* fbkg = new TF1("fbkg", Lan, 0.98, dU, 3);
   fbkg->SetParNames("N","A","B");
   fbkg->SetParameters(1., 0.493, 1.24);
   fbkg->SetLineWidth(2);
...
//    fbkg->SetParameters(1., -0.4, -5.7);
   fbkg->SetParameters(1., 0.01, -4);
...
//-------------------------------------------------------------------------
   ...
   hst->SetMaximum( 1.4*hst->GetMaximum() );

   // Fit KKeta background by reversed Argus function
   auto Lan = [](const double* x,const double* p) -> double {
      return bW*p[0]*RevArgusN(x[0],p[1],p[2],dL,dU);
   };

   TF1* fbkg = new TF1("fbkg", Lan, 0.98, dU, 3);
   fbkg->SetParNames("Nbkg","A","B");
   fbkg->SetParameters(5e3, 0.5, 1.2); //2009
//    fbkg->SetParameters(2e4, 0.5, 1.2); // 2012
   fbkg->FixParameter(1, 0.5); // A
//    fbkg->FixParameter(2, 1.25);  // B
   ...
   TLegend* leg = new TLegend(0.105,0.75,0.5,0.895);
   leg->AddEntry(hst,title.c_str(),"LEP");
   leg->AddEntry(fbkg,"Reversed Argus func.","L");
   leg->Draw();
...
//-------------------------------------------------------------------------
   ...
   TF1* fbkg = new TF1("fbkg", Lan, 0.98, dU, 3);
   fbkg->SetParNames("Nbkg","A","B");
   fbkg->SetParameters(2e4, 0.5, 1.2);
   fbkg->FixParameter(1, 0.5); // A=0.5
//    fbkg->FixParameter(2, 0.);
...
//-------------------------------------------------------------------------
void data_fit(string fname, string title, int date, string pdf="") {
//-------------------------------------------------------------------------
   ...
   auto Lsum = [](double* x,double* p) -> double {
      return bW*p[0]*( BreitWignerGaussN(x[0],p[1],p[2],p[3]) +
                       p[4]*RevArgusN(x[0],p[5],p[6],dL,dU)     );
   };

   TF1* fall = new TF1("fall", Lsum, dL, dU, 7);
   fall->SetParNames("Nphi","Mphi","Gphi","Sigma",
                     "Fbkg","A","B");
   if ( date == 2009 ) {
      fall->SetParameters(1.e3, Mphi,Gphi,1.4e-3, 0.05, 0.5,0.);
   } else { // 2012
      fall->SetParameters(2.5e3, Mphi,Gphi,1.1e-3, 0.06, 0.5,0.);
   }

//    fall->SetParLimits(4, 0.,1.e9); // Nbkg
//    fall->FixParameter(1, Mphi);
   fall->FixParameter(2, Gphi);
   fall->FixParameter(5, 0.494);   // A
   fall->FixParameter(6, 1.25);    // B
   ...
   auto Lbkg = [par](double* x,double* p) -> double {
      return bW*par[0]*par[4]*RevArgusN(x[0],par[5],par[6],dL,dU);
   };
   ...
   TLegend* leg = new TLegend(0.62,0.34,0.95,0.52);
   leg->AddEntry(hst,title.c_str(),"LEP");
...
double IntfrBWAR( double m,
                  ...
                  double A, double B, double L, double U, // Argus
...
// Return value:
//      ret_type =    0      1     2       3
//                    Sum    B-W   Argus   Interference
//----------------------------------------------------------------------
   static const double BWnorm = 1.028250; // see test_BreitWigner();
   ...
   // background:
   double Ar = Nr * Fb * RevArgusN(m,A,B,L,U);
...
double IntfrBWARG( double m,
                   double sigma,                     // Gauss
                   double A, double B,               // Argus
                   double Nr, double Fb, double Ang, // interference
                   int ret_type = 0,                 // what to return
                   double mphi=Mphi                  // phi-mass
                 ) {
//----------------------------------------------------------------------
// Function Fun(x) folded with a normal distribution:
//
...
//----------------------------------------------------------------------
   ...
   auto Lint = [=](double t) -> double{ // t == (X'-X)/sigma
      return exp(-0.5*t*t) *
      IntfrBWAR( (m+t*sigma),mphi,Gphi, A,B,dL,dU, Nr,Fb,Ang, ret_type );
   };
...
//-------------------------------------------------------------------------
   ...
   auto Lintfr = [](const double* x,const double* p) -> double {
      return IntfrBWARG( x[0], p[0], p[1],p[2],
                         p[3],p[4],p[5], int(p[6]) );
   };

   TF1* fun = new TF1("fun", Lintfr, bL, dU, 7);
   fun->SetParNames("Sigma", "A", "B", "Nphi","Fbkg","Ang","T");
   fun->SetParameters(1.2e-3, 0.5, 0.,  1.,0.05,0., 0);
   ...
   c1->cd();
   gPad->SetGrid();
   fun->SetParameter(5, 0); // Ang
   fun->DrawCopy();
//    ymax = fun->GetMaximum();

   fun->SetParameter(6, 2); // Argus
   fun->SetLineColor(kBlue);
   fun->SetLineStyle(kDashed);
   fun->DrawCopy("SAME");

   fun->SetParameter(6, 1); // BW
   fun->SetLineColor(kGreen);
   fun->DrawCopy("SAME");

   fun->SetParameter(6, 3); // interference
...
//-------------------------------------------------------------------------
   ...
   auto Lintfr = [](const double* x,const double* p) -> double {
      return bW*IntfrBWARG( x[0], p[0], p[1],p[2],
                            p[3],p[4],p[5], int(p[6]) );
   };

//    auto LintfrM = [](const double* x,const double* p) -> double {
//       return bW*IntfrBWARG( x[0], p[0], p[1],p[2],
//                             p[3],p[4],p[5], int(p[6]), p[7] );
//    };

   TF1* fun = new TF1("fun", Lintfr, bL, dU, 7);
//    TF1* fun = new TF1("fun", LintfrM, bL, dU, 8);
   fun->SetParNames("Sigma", "A", "B", "Nphi","Fbkg","Ang","T");
//    fun->SetParNames("Sigma", "A", "B", "Nphi","Fbkg","Ang","T","Mphi");
   if ( date == 2009 ) {
      fun->SetParameters(1.4e-3, 0.5, 1.4,  840., 0.007, 0.,  0);
//       fun->SetParameters(1.4e-3, 0.5, 1.4,  830., 0.01, 0.,  0, Mphi);
   } else { // 2012
      fun->SetParameters(1.1e-3, 0.5, 1.4, 2540., 0.02, 0.,  0);
//       fun->SetParameters(1.1e-3, 0.5, 1.4, 2500., 0.03, 0.,  0, Mphi);
   }

   fun->FixParameter(6, 0); // MUST BE FIXED!
   gStyle->SetOptFit(112);  // print fixed parameters!
//    gStyle->SetFitFormat(".4f");

   fun->FixParameter(1, 0.5);  // A
   fun->FixParameter(2, 1.4);   // B
//    fun->FixParameter(0, 1.13e-3); // Sigma
//    fun->FixParameter(5, 0.); // Ang
//    fun->SetParLimits(5,-0.1147,0.1147);
...
//    res->Print();
   ...
   auto Fdraw = [par](const double* x,const double* p) -> double {
      return bW*IntfrBWARG( x[0], par[0], par[1],par[2],
                            par[3],par[4],par[5], int(p[0]) );
//                             par[3],par[4],par[5], int(p[0]), par[7] );
   };
...
//    TLegend* leg = new TLegend(0.62,0.43,0.98,0.68); // 111
   TLegend* leg = new TLegend(0.62,0.32,0.98,0.57);
   leg->AddEntry(hst,title.c_str(),"LEP");
...
//-------------------------------------------------------------------------
   ...
   fun->SetParNames("Sigma", "A", "B", "Nphi","Fbkg","Ang","T");
   if ( date == 2009 ) {
      fun->SetParameters(1.4e-3, 0.495,1.23,  950.,  0.03, -M_PI/2,  0);
   } else { // 2012
      fun->SetParameters(1.1e-3, 0.495,1.23, 2800.,  0.05, -M_PI/2,  0);
   }

   fun->FixParameter(6, 0); // MUST BE FIXED!
   fun->FixParameter(1, 0.494);  // A
   fun->FixParameter(2, 1.25);   // B
   ...
   // angles in degrees
   int angmin = -90, angmax = 90, angstep = 5;
   ...
   for( int ang = angmin; ang <= angmax; ang+=angstep ) {
      fun->FixParameter(5, M_PI*ang/180.); // Ang -> radians
      ...
      la->SetLabel(Form("Ang= %i deg",ang));
      leg->Draw();
...
class myChi2 {
   public:
      ...
      double operator() (const double* p) {
         ...
         // A,B,Fbkg,Ang - common parameters
         auto F1 = [p](double m) -> double {
            return bW*IntfrBWARG( m, p[0], p[2],p[3], p[4], p[6],p[7], 0 );
         };
         auto F2 = [p](double m) -> double {
            return bW*IntfrBWARG( m, p[1], p[2],p[3], p[5], p[6],p[7], 0 );
         };
...
//    }
   ...
   vector<string> par_name
        {"sig12", "sig09", "A", "B",  "N12", "N09", "Fbkg", "Ang" };
   vector<double> par_ini
        { 1.1e-3, 1.4e-3,  0.5, 1.4,  2550.,  820.,  0.020,    0. };
...
//    fitter.Config().ParSettings(5).Fix(); // N09 debug!

   // fix  parameters
   fitter.Config().ParSettings(2).SetValue(0.5);              // A
   fitter.Config().ParSettings(2).Fix();
//    fitter.Config().ParSettings(2).SetStepSize(0.001);
//    fitter.Config().ParSettings(2).SetLimits(1.e-3,10.);
   fitter.Config().ParSettings(3).SetValue(1.4);
   fitter.Config().ParSettings(3).Fix();                        // B
//    fitter.Config().ParSettings(3).SetStepSize(0.01);
//    fitter.Config().ParSettings(3).SetLimits(-10.,10.);
//    fitter.Config().ParSettings(7).SetValue(0.);                 // Ang
...
//    fitter.CalculateHessErrors();
   ...
   string Ta( Form("A =\t %.3f #pm  %.3f",par[2],er_par[2]) );
   string Tb( Form("B =\t %.3f #pm  %.3f",par[3],er_par[3]) );
   string Tn12( Form("N_{#phi}(2012) = %.1f #pm  %.1f",par[4],er_par[4]) );
   string Tn09( Form("N_{#phi}(2009) = %.1f #pm  %.1f",par[5],er_par[5]) );
   string Tbkg( Form("Fbkg = %.4f #pm  %.4f",par[6],er_par[6]) );
   string Tang( Form("Ang = %.3f #pm  %.3f",par[7],er_par[7]) );
...
//    TCanvas* c1 = new TCanvas("c1","...",0,0,900,900); // slide
   ...
   // draw components of Interference
   auto F1 = [par](const double* x,const double* p) -> double {
      return bW*IntfrBWARG( x[0], par[0], par[2],par[3],
                            par[4],par[6],par[7], int(p[0]) );
   };
   auto F2 = [par](const double* x,const double* p) -> double {
      return bW*IntfrBWARG( x[0], par[1], par[2],par[3],
                            par[5],par[6],par[7], int(p[0]) );
   };
...
//    TLegend* leg = new TLegend(0.51,0.40,0.89,0.89); // slide
   TLegend* leg = new TLegend(0.51,0.50,0.89,0.89); // memo
...
//    hst1->GetXaxis()->SetTitleOffset(0.9);
//    hst1->GetYaxis()->SetTitleOffset(0.7);
...
//    hst2->GetXaxis()->SetTitleOffset(0.9);
//    hst2->GetYaxis()->SetTitleOffset(0.7);
...
//    TPaveText* pt = new TPaveText(0.51,0.26,0.89,0.999,"NDC"); // slide
   ...
   pt->SetTextAlign(12);
   pt->SetTextFont(42);
//    pt->AddText( "Combined Fit" );
   pt->AddText( Tchi2.c_str() );
   pt->AddText( Ta.c_str() );
   pt->AddText( Tb.c_str() );
   pt->AddText( Tbkg.c_str() );
   pt->AddText( Tang.c_str() );
//    pt->AddLine(0.,0.444,1.,0.444 );
   pt->AddLine(0.,0.454,1.,0.454 );
   pt->AddText( Tn12.c_str() );
   pt->AddText( Ts12.c_str() );
//    pt->AddLine(0.,0.222,1.,0.222 );
   pt->AddLine(0.,0.232,1.,0.232 );
...
//    string pdf = string("mkk")+((id==0) ? "09" : "12")+"_fit1.pdf";
//    data_fit(fnames.at(id),titles.at(id),2009+id*3,pdf);

// ------------- data: interference ---------------
//    test_Intfr()
//    string pdf = string("mkk")+((id==0) ? "09" : "12")+"_intfrAB.pdf";
//    data_Intfr_fit(fnames.at(id),titles.at(id),2009+id*3,pdf);

//    data_Intfr_scan(fnames.at(id),titles.at(id),2009+id*3);

//    combine_Intfr();
   combine_Intfr("mkk_cf.pdf");
