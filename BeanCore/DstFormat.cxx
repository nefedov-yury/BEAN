...
// DstFormat                                                            //
//                                                                      //
// This is description of the format of DST and few _base_ functions    //
//                                                                      //
//////////////////////////////////////////////////////////////////////////
#include <iostream>
#include <iomanip>
...
//-----------------------------------------------------------------------------
{
   ...
   // FIXME: crash on workers in case of cluster PROOF
//    if (!fChain) return;
...
//-----------------------------------------------------------------------------
{
   ...
   while ((mdcTrack = (TMdcTrack*)mdcTrackIter.Next())) {
     ...
     if( Verbose() ) {
       cout << " Helix parameters: d0, phi0, 1/Pt, dz, tan(lambda)" << endl;
       for(int i = 0; i < 5; i++) {
...
//-----------------------------------------------------------------------------
{
   ...
   TMdcDedx* mdcDedx = 0;
   while ((mdcDedx = (TMdcDedx*)mdcDedxIter.Next())) {
     ...
     cout << endl;
//      cout << " chi2= " << mdcDedx->chiE() << " : "
//           << mdcDedx->chiMu() << " : " << mdcDedx->chiPi() << " : "
//           << mdcDedx->chiK() << " : " << mdcDedx->chiP() << endl;
   }
}
   ...
   while ((mdcKalTrack = (TMdcKalTrack*)mdcKalTrackIter.Next())) {
     ...
     for(int ip = 0; ip < 5; ip++) { // e/mu/pi/K/p
        if( ip == 2 || Verbose() ) { // always print for pi
          cout << "  ----------------- " << endl;
          cout << " | " << setw(8) << pid_name[ip] << " fit    |" << endl;
          cout << "  ----------------- " << endl;
          cout
#if BOSS_VER >= 655
...
//-----------------------------------------------------------------------------
{
   ...
   TExtTrack* extTrack = 0;
   while ((extTrack = (TExtTrack*)extTrackIter.Next())) {
     ...
     if( Verbose() ) {
       cout << "       VolumeName= " << extTrack->GetTof1VolumeName()
            << " VolumeNumber= " << extTrack->GetTof1VolumeNumber() << endl;
     ...
     if( Verbose() ) {
       cout << "       VolumeName= " << extTrack->GetTof2VolumeName()
            << " VolumeNumber= " << extTrack->GetTof2VolumeNumber() << endl;
       cout << "       SigmaAlong: X,Y,Z,T= "
     ...
     if( Verbose() ) {
       cout << "       VolumeName= " << extTrack->GetEmcVolumeName()
            << " VolumeNumber= " << extTrack->GetEmcVolumeNumber() << endl;
     ...
     if( Verbose() ) {
       cout << "       VolumeName= " << extTrack->GetMucVolumeName()
            << " VolumeNumber= " << extTrack->GetMucVolumeNumber() << endl;
       cout << "       SigmaAlong: X,Y,Z,T= "
...
//-----------------------------------------------------------------------------
{
   ...
   if( !Verbose() ) {
     ...
     MaxEmcTracks = NmdcTracks+3; // max number of emc clusters to print
   }
   ...
   for( int condition = 0; condition < 2; condition++) {
     ...
     TEmcTrack* emcTrack = 0;
     while ((emcTrack = (TEmcTrack*)emcTrackIter.Next())) {
       if( condition == 0 ) {
         if( emcTrack->trackId() < 0 ) continue;
       } else {
         if( emcTrack->trackId() >= 0 ) continue;
         if( i_emc_trk >= MaxEmcTracks ) {
           cout << " ... skip " << m_emcTrackCol->GetEntries() - i_emc_trk
   ...
   while ((mucTrack = (TMucTrack*)mucTrackIter.Next())) {
     ...
     if( Verbose() ) {
       ...
       cout << " BarrelLastLayer " << mucTrack->brLastLayer()
            << " EndcapLastLayer= " << mucTrack->ecLastLayer() << endl;
       cout << " numLayers= " << mucTrack->numLayers()
            << " maxHitsInLayer= " << mucTrack->maxHitsInLayer() << endl;
     }
...
//-----------------------------------------------------------------------------
{
   ...
   if( isVtxValid ) {
     ...
     cout << " +++++ nTracks= " << m_evtRecPrimaryVertex->nTracks()
          << " trkID's= ";
     const vector<Int_t>& trkIDs = m_evtRecPrimaryVertex->trackIdList();
     copy(trkIDs.begin(), trkIDs.end(), ostream_iterator<int>(cout, " "));
     cout << endl;
     cout << "       fitMethod= " << m_evtRecPrimaryVertex->fitMethod()
     ...
     if( Verbose() ) {
       ...
       for(int i = 0, k = 0; i < 3; i++) {
          ...
          if( i != 0 ) cout << setw(15*i) << " : ";
          for(int j = i; j < 3; j++,k++) {
               cout << setw(12) << m_evtRecPrimaryVertex->errorVertex(k)
                    << " : ";
          }
          cout << endl;
       }
...
//-----------------------------------------------------------------------------
{
   ...
   while ((evtRecTrack = (TEvtRecTrack*)evtRecTrackIter.Next())) {
     ...
     if( !tof_id.empty() ) {
       cout << "      tofTracks= ";
       copy(tof_id.begin(), tof_id.end(), ostream_iterator<int>(cout, " "));
       cout << endl;
     }
...
//-----------------------------------------------------------------------------
{
   ...
   while ((evtRecVeeVertex = (TEvtRecVeeVertex*)evtRecVeeVertexIter.Next())) {
     ...
     if( Verbose() ) {
       cout << "            nTracks= " << evtRecVeeVertex->nTracks()
...
//-----------------------------------------------------------------------------
{
   ...
   TEvtRecEtaToGG* evtRecEtaToGG = 0;
...
//-----------------------------------------------------------------------------
{
   ...
   while ((evtRecDTag = (TEvtRecDTag*) evtRecDTagIter.Next())) {
     ...
     if( !otrk.empty() ) {
       ...
       copy(otrk.begin(), otrk.end(), ostream_iterator<int>(cout, " "));
       cout << endl;
     }
     ...
     if( !oshw.empty() ) {
       ...
       copy(oshw.begin(), oshw.end(), ostream_iterator<int>(cout, " "));
       cout << endl;
     }
     ...
     if( !piid.empty() ) {
       cout << "      vector<pionId>= ";
       copy(piid.begin(), piid.end(), ostream_iterator<int>(cout, " "));
       cout << endl;
     }
...
//-----------------------------------------------------------------------------
{
   ...
   TMdcMc* mdcMc = 0;
   while ((mdcMc = (TMdcMc*)mdcMcHitIter.Next())) {
...
//-----------------------------------------------------------------------------
{
   ...
   TEmcMc* emcMc = 0;
   while ((emcMc = (TEmcMc*)emcMcHitIter.Next())) {
     ...
     if( Verbose() ) {
       ...
       for(it = emcMcHitMap.begin(); it != emcMcHitMap.end(); it++) {
...
//-----------------------------------------------------------------------------
{
   ...
   TTofMc* tofMc = 0;
   while ((tofMc = (TTofMc*)tofMcHitIter.Next())) {
...
//-----------------------------------------------------------------------------
{
   ...
   TMucMc* mucMc = 0;
   while ((mucMc = (TMucMc*)mucMcHitIter.Next())) {
...
//-----------------------------------------------------------------------------
{
   ...
   if( m_trigData && m_trigData->IsA() == TTrigData::Class() ) {
     ...
     if( Verbose() ) {
       ...
       if( trg_cnd ) {
         cout << " TrigCondition= ";
         copy(trg_cnd, trg_cnd+48, ostream_iterator<int>(cout, " "));
         cout << endl;
       }
       const int* trg_chan = m_trigData->getTrigChannel();
       if( trg_cnd ) {
         cout << " TrigChannel= ";
         copy(trg_chan, trg_chan+16, ostream_iterator<int>(cout, " "));
         cout << endl;
       }
     }
   }
...
// Clear() functions are empty !
   if( m_TEvtHeader    ) m_TEvtHeader->Clear();

   if( m_TDstEvent     ) {
     m_TDstEvent->Clear();

//      ((TObjArray*)m_TDstEvent->getMdcTrackCol())->Delete();
     const_cast<TObjArray*>(m_TDstEvent->getMdcTrackCol())->Delete();
     const_cast<TObjArray*>(m_TDstEvent->getMdcDedxCol())->Delete();
     const_cast<TObjArray*>(m_TDstEvent->getMdcKalTrackCol())->Delete();
     const_cast<TObjArray*>(m_TDstEvent->getTofTrackCol())->Delete();
     const_cast<TObjArray*>(m_TDstEvent->getExtTrackCol())->Delete();
     const_cast<TObjArray*>(m_TDstEvent->getEmcTrackCol())->Delete();
     const_cast<TObjArray*>(m_TDstEvent->getMucTrackCol())->Delete();
   }
   ...
   if( m_TMcEvent      ) {
     ...
     const_cast<TObjArray*>(m_TMcEvent->getMdcMcHitCol())->Delete();
     const_cast<TObjArray*>(m_TMcEvent->getEmcMcHitCol())->Delete();
     const_cast<TObjArray*>(m_TMcEvent->getTofMcHitCol())->Delete();
     const_cast<TObjArray*>(m_TMcEvent->getMucMcHitCol())->Delete();
     const_cast<TObjArray*>(m_TMcEvent->getMcParticleCol())->Delete();
   }
