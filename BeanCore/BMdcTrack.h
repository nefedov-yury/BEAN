...
class BMdcTrack : public TMdcTrack 
{
 public:
  ...
  const CLHEP::HepVector        helix()                 const;
  const CLHEP::HepSymMatrix     err()                   const;
  const CLHEP::Hep3Vector       p3()                    const;
  const CLHEP::HepLorentzVector p4(const double mass)   const;
