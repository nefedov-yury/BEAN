...
//--------------------------------------------------------------------
{
  ...
  if( mdc_trk_id >= 0 ) {
    ...
    if( tmp ) mdc_trk = new RecMdcTrack(tmp);
  }
  ...
  if( mdc_dedx_trk_id >= 0 ) {
    ...
    if( tmp ) mdc_dedx = new RecMdcDedx(*tmp); // just copy
  }
  ...
  if( id_size > 0 ) {
    ...
    for(UInt_t i = 0; i < id_size; i++) {
       TTofTrack* tmp = (TTofTrack*) m_tofTrackCol->At(tof_trk_id[i]);
       if( tmp ) tof_trk.push_back( new RecTofTrack(tmp) );
    }
  }
  ...
  if( ext_trk_id >= 0 ) {
    ...
    if( tmp ) ext_trk = new RecExtTrack(tmp);
  }
  ...
  if( emc_trk_id >= 0 ) {
    ...
    if( tmp ) emc_trk = new RecEmcShower(tmp);
  }
  ...
  if( muc_trk_id >= 0 ) {
    ...
    if( tmp ) muc_trk = new RecMucTrack(*tmp); // just copy
  }
}
