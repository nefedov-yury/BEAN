...
//-----------------------------------------------------------------------------
{
   if( selector->Verbose() ) cout << " BhabhaStartJob() " << endl;
...
//    his1.resize(100,(TH2D*)0);
   ...
   htheta1 =  new TH1D("htheta1", "90 - (theta(1)-theta(2))/2", 360, 0, 180);

   h1a1 =  new TH1D("h1a1_Theta1p2", "1. a.1) : #theta_{1}+#theta_{2} ( 20<#Theta*<40 ) ", 200, 178, 182);
   h1a2 =  new TH1D("h1a2_Theta1p2", "1. a.2)  : #theta_{1}+#theta_{2} ( 40<#Theta*<90 ) ", 200, 178, 182);
   h1b1 =  new TH1D("h1b1_Phi1m2", "1. b.1) : |#phi_1-#phi_2| ( 20<#Theta*<40 ) ", 200, 177, 183);
   h1b2 =  new TH1D("h1b2_Phi1m2", "1. b.2) : |#phi_1-#phi_2|  ( 40<#Theta*<90 ) ", 100, 178, 182);
   //1c theta*-bins
   h1c = new TH2D("h1c_Theta1p2_vs_ThetaSt", "1. c.1) ;#theta*;#theta_{1}+#theta_{2}", 
                        70, 20, 90, 100, 174, 186); //setOptionStat 1100011  !!!0.6
   //1d phi*-bins
   h1d = new TH2D("h1d_Theta1p2_vs_PhiSt", "1. d.1);#phi*;#theta_{1}+#theta_{2}",
                        180, -90, 90, 100,175, 185);
   
    //1. e)  Plot |Phi1-Phi2| in bins of Theta*
   h1e = new TH2D("h1e_Phi1m2_vs_ThetaSt", "1. e.1) ;#Theta*;|#phi_1-#phi_2|", 
                        140, 20, 90,   200, 178,  182); //setOptionStat 1100011  !!!0.6

	// 1. f) Plot |Phi1-Phi2| in bins of Phi, separately
   h1f1 = new TH2D("h1f1_Phi1m2_vs_PhiSt", "1. f.1) ( 20<#Theta*<40 )  ;#phi*;#|#phi_1-#phi_2|", 
                        180, -90, 90, 200, 176, 184); //setOptionStat 1100011  !!!0.6
                        
   h1f2 = new TH2D("h1f2_Phi1m2_vs_PhiSt", "1. f.2) ( 40<#Theta*<90 )  ;#phi*;#|#phi_1-#phi_2|", 
                        90,  -90, 90, 100, 179, 181); //setOptionStat 1100011  !!!0.6


   h3a1 = new TH2D("h3a1_DedxTheta", ";#theta;dE/dx", 200, 0, 180, 100,0.6,2.);
   h3a2 = new TH2D("h3a2_DedxPhi_fwd", "Endcap only:  |#Theta-90|>50;#varphi;dE/dx", 400, -180, 180, 100,0.6,2.);
   h3a3 = new TH2D("h3a3_DedxPhi", "Barrel only:  |#Theta-90|<50 ;#varphi;dE/dx", 400, -180, 180, 100,0.6,2.);
           
   h3d1 = new TH2D("h3d1_BetaTheta", ";#theta;#beta", 200, 0, 180, 400,-0.1,1.5);
   
   h3d2 = new TH2D("h3d2_BetaPhi", "Barrel only:  |#Theta-90|<50 ;#varphi;#beta", 400, -180, 180, 400,-0.1,1.5);
   h3d2a = new TH2D("h3d2a_BetaPhi", "Endcap only:  |#Theta-90|>50 ;#varphi;#beta", 400, -180, 180, 400,-0.1,1.5);
   
   h3h1 = new TH2D("h3h1_NMucLayersTheta", ";#theta;MUC Layers", 50, 0, 180, 10,0,10.);
   h3h2 = new TH2D("h3h2_NMucLayersPhi", ";#varphi;MUC Layers", 100, -180, 180, 10,0,10.);
   
   h3j = new TH2D("h3j_dT_Theta",";#theta; T - T_{exp}",90, 0, 180,1200,-2,10);
   h3k = new TH1D("h3k_dT","T - T_{exp}",1200,-2,10);
   
   
   h2a1 = new TH2D("h2a1_pelTheta", ";#theta;p(electron)", 50, 0, 180, 100,0,M_PI);
   h2a2 = new TH2D("h2a2_pposTheta", ";#theta;p(positron)", 50, 0, 180, 100,0,M_PI);
   ...
   h22a1 = new TH2D("h22a1_pelPhi", ";#varphi;p(electron)", 360, -180, 180, 400,0,2.2);
   h22a2 = new TH2D("h22a2_pposPhi", ";#varphi;p(positron)", 360, -180, 180, 400,0,2.2);

   h22a3 = new TH2D("h22a3_pPhi_fwd", "(|#Theta-90| > 50 );#varphi;p", 360, -180, 180, 800,-2.2,2.2);
   h22a4 = new TH2D("h22a4_pPhi", "(|#Theta-90| < 50 );#varphi;p", 360, -180, 180, 800,-2.2,2.2);


   h22c = new TH2D("h22c_EPhi", ";#varphi;E;", 360, -180, 180, 100,0,2.2);
...
//   haux1 = new TH2D("haux1_Phi1_Phi2", ";#varphi_1;#varphi_2;", 100,-M_PI,M_PI, 100,-M_PI,M_PI);
   ...
   h6b = new TH2D("h6b_relsigma_Pt",";Pt;sigma(Pt)/Pt",400,-2,2,500,0,0.1);

   hEc_phi =  new TH1D("E_c_phi", "|#phi_1-#phi_2| EndCap", 200, 177, 183);
   hEc_theta =  new TH1D("E_c_theta", "#theta_{1}+#theta_{2} EndCap", 200, 178, 182);
   hEb_phi =  new TH1D("E_b_phi", "|#phi_1-#phi_2| Barell", 200, 177, 183);
   hEb_theta =  new TH1D("E_b_theta", "#theta_{1}+#theta_{2} Barell", 200, 178, 182);
...
//				pid->useEmc()  |
	    ...
	    pid->identify(pid->onlyElectron() | pid->onlyMuon());    // seperate Pion/Kaon
	    pid->calculate(runNo);
...
// nef			trackInfo[i] = new DstEvtRecTracks((TEvtRecTrack*) m_TEvtRecObject->getEvtRecTrackCol()->At(i), m_TDstEvent);	
            ...
            if(kal[i]) kal[i]->setPidType(RecMdcKalTrack::electron); 
	    ...
	    if ( emc[i] ) {
                ...
                if ( kal[i] ){
                    ...
                    p4[i].SetPxPyPzE( 
                        mdc[i]->px(),mdc[i]->py(),mdc[i]->pz(),                    emc[i]->energy() );
			...
			for(vector<RecTofTrack* >::const_iterator it = trackInfo[i]->tofTrack().begin(); it != trackInfo[i]->tofTrack().end(); ++it) {
				...
				if  ( TofHitStatus::is_counter(  ((RecTofTrack*)(*it))->status() ) ) {                     
				  ...
				  tof_path_sum += 0.0 + ((RecTofTrack*)(*it))->path(); // there is not a lot of sense in average path					
				  tof_cnt += 1;
		...
		if (flag > 0 ) { //at least one with E>0.8*ebeam, p>0.8*pbeam
			// 2)
			if ( ((mdc[0])||(kal[0])) && ((mdc[1])||(kal[1])) && emc[0] && emc[1] ) {
				...
				if ( ( TMath::Abs(t1_p_t2 - M_PI) / M_PI  < 0.01 ) &&
					 ( TMath::Abs(p1_m_p2 - M_PI) / M_PI  < 0.01 ) ) {// Theta1+Theta2, Phi1-Phi2 close to 180
						for (i=0; i<2; i++) {			
							if (flags[i]) {  // for *one track* p>0.8*pbeam, E>0.8*ebeam.; [1-i] - another track
								if (mdc[1-i]->charge() == -1 ) {
									h2a1->Fill(RtoD(theta[1-i]), p[1-i] );//2.a) <p(electron)> versus Theta,
									h22a1->Fill(RtoD(phi[1-i]), p[1-i] );
									
								} else {
									h2a2->Fill(RtoD( theta[1-i]), p[1-i] );//2.a) <p(positron)> versus Theta
            ...
            htheta1->Fill(theta_mean);
				...
				Double_t th_star = M_PI/2 - TMath::Abs(theta[0]  - theta[1])/2;  //	Theta*=90-|90-0.5*(180+Theta1-Theta2)|
				...
				if ( (th_star > DtoR(20)) && (th_star <  DtoR(40)) ) {
					h1a1->Fill(  RtoD(theta[0] +  theta[1]) ); // 1. a.1) Theta1+Theta2, 20<Theta*<40
					h1b1->Fill(RtoD( TMath::Abs( phi[0] - phi[1]))); // 1. b.1) |Phi1-Phi2|, 20<Theta*<40
					h1f1->Fill(RtoD(phi_star), RtoD(TMath::Abs( phi[0] - phi[1]))); // 1. f.1) Plot |Phi1-Phi2| in bins of Phi
...
//                                1.0/ tan(phi[0]) - 1.0/ tan(phi[1])  );
				...
				h1c->Fill(RtoD(th_star), RtoD(theta[0] +  theta[1])); // 1. c) Plot Theta1+Theta2 in small bins of Theta*
				
				
				
				h1d->Fill(RtoD(phi_star),RtoD( theta[0] +  theta[1])); // 1. d) Do the same in bins of Phi*
				
				h1e->Fill(RtoD(th_star), RtoD(TMath::Abs(phi[0] -  phi[1]))); //1. e) Plot |Phi1-Phi2| in bins of Theta*
					 ...
					 ( TMath::Abs(p1_m_p2 - M_PI) / M_PI  < 0.01 ) ) // Theta1+Theta2, Phi1-Phi2 close to 180
                {
                  ...
                  if (kal[0] && kal[1]) {
						...
						for (i = 0; i < 2; i++){
						    if( dedx[i] )				  
							h3a1->Fill( RtoD(theta[i]) , dedx[i]->normPH() ); // 3.a-b) <dE/dx> versus Theta, <dE/dx> versus Phi
							
							//**** TRecMdcKalTrack - where is pathSM ?!
							Double_t dT = tof[i] - mdc_path[i]*1.0 / (speed_of_light/1E7); // T - T_expected 
							h3j->Fill( RtoD(theta[i]) ,dT);
							h3k->Fill( dT);
							
							
							h3d1->Fill( RtoD(theta[i]) , mdc_path[i]*1.0 / tof[i] /(speed_of_light/1E7) ); // def) beta versus Theta, versus Phi
							
							if ( RtoD( TMath::Abs( theta[i] - M_PI/2 ))  < 50 ) { //barell
							    h3d2->Fill( RtoD(phi[i])   , mdc_path[i]*1.0 / tof[i] /(speed_of_light/1E7) );
							    if( dedx[i] )				  
							    h3a3->Fill(RtoD( phi[i])   , dedx[i]->normPH() );
							} else { //endcap
							    h3d2a->Fill( RtoD(phi[i])   , mdc_path[i]*1.0 / tof[i] /(speed_of_light/1E7) );
							    if( dedx[i] )				  
							...
							h3h2->Fill( RtoD(phi[i])   , muc[i]->numLayers() );  //ghi) number of MUC layers versus Theta, versus Phi
							}
							haux1 -> Fill( mdc[i]->x() );
							haux2 -> Fill( mdc[i]->y() );
							haux3 -> Fill( mdc[i]->z() );
                     ...
                     // 							h6b-> Fill( 1./kal[i]->getZHelixE(2) ,sqrt( kal[i]->getZErrorE(2,2) ) / fabs(kal[i]->getZHelixE(2)) );
                        ...
                        h6b-> Fill( 1./kal[i]->kappa() ,sqrt( Err[2][2] ) / fabs(kal[i]->kappa()) );
                      }
