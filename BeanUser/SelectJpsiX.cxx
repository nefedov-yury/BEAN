...
//=====================================================================================================================================//
//                                                                                                                                                                                 //
// 9-06-2020  (+BOSS 7.0.5)              !!! delete  3.0-3.2                                                                                                                      //
//new mass cuts 5-11-2020                                                                                                                                                                         //
//=====================================================================================================================================//
...
//-----------------------------------------------------------------------------
static int ParticleTypesMC(const ReadDst* selector)
//-----------------------------------------------------------------------------
{    
  if ( !isMC ) return -1;
  const TMcEvent* m_TMcEvent = selector->GetMcEvent();
  ...
  while ( TMcParticle* part = 
    ...
    //PDG:  e- = 11, mu- = 13, J/psi = 443, psi(2S) = 100443, psi(3770) = 30443, psi(4040) = 9000443,   chi_c1 = 20443, chi_c2 = 445
...
//-------------------------------------------------------------------------------- Get event information-------------------------------------------------------
  ...
  if ( isMC ) {
    ...
    mc_type = ParticleTypesMC(selector);
  ...
  int nGood = S2K2piG.nGood();
  int nPos = S2K2piG.nPos();
  int nNeg = S2K2piG.nNeg();
  his1[5]->Fill(nGood);
  his1[6]->Fill(Ncharge);
  ...
  //for preselection J/psi + anything / phi,  Ks, Lambda
  ...
  for(int i = 0; i < nPos; i++){
	...
	//=============================================================================================== J/psi -> e+e-
	//for ee candidates
	if( (EnergyEMCtrkp > 0.8) && (EnergyEMCtrkm > 0.8) ){
	  
	//	if( (EoverPp > 0.7) && (EoverPm > 0.7) ){

	  mdcKalTrkp->setPidType(RecMdcKalTrack::electron);
	  Hep3Vector p3ep(mdcKalTrkp->px(), mdcKalTrkp->py(), mdcKalTrkp->pz());
	  double eep = sqrt( p3ep.mag2() + me*me );
	  int tmp_iep  = S2K2piG.qtrksPos[i];
	  HepLorentzVector tmp_pep  =  HepLorentzVector( p3ep, eep );
     
	 
	  
	  mdcKalTrkm->setPidType(RecMdcKalTrack::electron);
	  Hep3Vector p3em(mdcKalTrkm->px(), mdcKalTrkm->py(), mdcKalTrkm->pz());
	  double eem = sqrt( p3em.mag2() + me*me );
	  int tmp_iem  = S2K2piG.qtrksNeg[j];
	  HepLorentzVector tmp_pem =  HepLorentzVector( p3em, eem );
	  

	  //for Bhabha rejection
	  double Thetap = mdcTrkp->theta();
	  double cosThetap = cos(Thetap);
	  double Thetam = mdcTrkm->theta();
	  double cosThetam = cos(Thetam);
	  double BwnAngCut = 179.; //degree
	  
	  double cos2trks = ( p3ep.x()*p3em.x() +p3ep.y()*p3em.y() + p3ep.z()*p3em.z() ) /( p3ep.mag()*p3em.mag() );

	  
	  
	  
	  HepLorentzVector Plle = tmp_pep + tmp_pem;
	  
	 
	  //Jpsi
	  if(Plle.m() > 2.8  &&  Plle.m() < 3.4){  
	    ...
	    //!Bhabha
	    if( (fabs(cosThetap) < 0.81) && (fabs(cosThetam) < 0.81) && (cos2trks > cos(BwnAngCut*M_PI/180.)) ){
	      
	      if(nGood!=2 && (nPos!=2 || nNeg!=2) ){ his1[422]->Fill(Plle.m()); }
	      nlleEPcomb++;
	      
	      
	      //selection flag
	      ElectronsFromJpsi=true;
	      
	      //--------------------------------------- J/psi candidate -----------------------
	      S2K2piG.qeps.push_back(tmp_iep);
	      S2K2piG.qems.push_back(tmp_iem);
	      S2K2piG.Peps.push_back(tmp_pep);
	      S2K2piG.Pems.push_back(tmp_pem);  
	      //-------------------------------------------------------------------------------
	      ...
	      his1[414]->Fill(Ptrkp);
	      his1[414]->Fill(Ptrkm);
	      his1[415]->Fill(EnergyEMCtrkp);
	      his1[415]->Fill(EnergyEMCtrkm);
	      his1[426]->Fill( cos2trks  );
...
//	if( (EnergyEMCtrkp < 0.5) && (EnergyEMCtrkm < 0.5) ){
	...
	//total check
	  ...
	  if(Pllmu.m() > 2.8  &&  Pllmu.m() < 3.4){
	    ...
	    if( Pllmu.m() > (mJpsi - 3.*sigma_Jpsi)  &&  Pllmu.m() < (mJpsi + 3.*sigma_Jpsi) ){
	      ...
	      //---------------------------------------------------------------------- Missing Mass
	      P_missing = ecms - Pllmu;
	      
	      his1[260]->Fill(P_missing.z());
	      his1[261]->Fill(P_missing.perp());
	      his1[262]->Fill(P_missing.m());
	      his1[276]->Fill(P_missing.m2());
	      his2[465]->Fill(P_missing.z(), P_missing.m2());
	      his2[466]->Fill(P_missing.z(), P_missing.perp());
	      
	      
	      if(Ecms < 4.){
		if( fabs(P_missing.z()) < 0.6 ) his1[277]->Fill(P_missing.m2());
	      }else{
		if( fabs(P_missing.z()) < 1.2 ) his1[277]->Fill(P_missing.m2());
	      }
	      //=========================================
  ...
  //------------------------------------------------------------------------------------------
  if( (nGam > 1) && MuonsFromJpsi3sigma ){

    his2[467]->Fill(P_missing.z(), P_missing.m2());
    his2[468]->Fill(P_missing.z(), P_missing.perp());
  }
  //==========================================
  ...
  if(MuonsFromJpsi){
	...
	for(int i = 0; i < nPos; i++){
	  for(int j = 0; j < nNeg; j++){
		...
		//non-ISR  psi'
		if(( nPos==2 && nNeg==2 && nGam > 1 ) || (nPos > 2) || (nNeg > 2)){ his1[526]->Fill( invJpsi2pi.m() );   his2[463]->Fill( invJpsi2pi.m(), inv2pi.m() );}

		
		//ISR psi'
		if( nPos==2 && nNeg==2 && nGam < 2 ){ his1[556]->Fill( invJpsi2pi.m() ); his2[464]->Fill( invJpsi2pi.m(), inv2pi.m() );}


		
		
		//psi'X
		if( (invJpsi2pi.m()  > 3.67)  &&  (invJpsi2pi.m() < 3.7) ){
		  ...
		  if( nPos==2 && nNeg==2 && nGam < 2){ isTrk4psi=true; }
    ...
    if(Jpsi2piFromPsi2S){
      ...
      if(isGoodMCevent) {
	if((nPos==2 && nNeg==2 && nGam > 1) || (nPos > 2) || (nNeg > 2)){Psi2SMomentumHistMC( selector, 1); }
      }//end if(isGoodMCevent)

      his1[107]->Fill(5);

      if((nPos==2 && nNeg==2 && nGam > 1) || (nPos > 2) || (nNeg > 2)){ his1[622]->Fill(nGood);}


      
     
      if( nPos==2 && nNeg==2 && nGam < 2 ){ 
    ...
    if( nPos==2 && nNeg==2 && nGam < 2 ){
      ...
      //mass cut
      if( (invJpsi2pi.m()  > 3.64)  &&  (invJpsi2pi.m() < 3.73) ){
...
*/
  ...
  if(MuonsFromJpsi){
	...
	for(int i = 0; i < nGam; i++){
	  ...
	  //chi_c1  -------------------------------------
	  if( (invJpsiGamma.m() > 3.42)  &&  (invJpsiGamma.m() < 3.56) ){
	  ...
	  //chi_c2  -------------------------------------
	  if( (invJpsiGamma.m() > 3.4)  &&  (invJpsiGamma.m() < 3.6) ){
...
#ifdef  MC_chi_c1
  if( JpsiGammaFromChiC1 ){ chic12MomentumHistMC( selector, 1); }
#endif

#ifdef  MC_chi_c2 
  if( JpsiGammaFromChiC2 ){ chic12MomentumHistMC( selector, 1); }
#endif
