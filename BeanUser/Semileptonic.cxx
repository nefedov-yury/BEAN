...
static void smartFillHist(std::string name, double value, bool is_good, bool is_excbad = false, bool has_mc = true) {
...
static void smartBookHistImpl(VecObj & his1, TString title, TString description, int nBins, double low, double high ) {
      //~ cout << title << endl;
      his1.push_back( new TH1D( title , TString::Format(description, "all"), nBins, low, high));
      his1.push_back( new TH1D( title + "_good" , TString::Format(description, "signal"), nBins, low, high));
      his1.push_back( new TH1D( title + "_bad"  , TString::Format(description, "background"), nBins, low, high));
      his1.push_back( new TH1D( title + "_exbad"  , TString::Format(description, "exclusive background"), nBins, low, high));
}

static void smartBookHist(VecObj & his1, VecObj & his_dtag, TString title, TString description, int nBins, double low, double high ) {
      ...
      smartBookHistImpl(his_dtag, title + "_+" , description, nBins, low, high);
      smartBookHistImpl(his_dtag, title + "_-" , description, nBins, low, high);
...
static void smartBookHist(VecObj & his_init, VecObj & his, VecObj & his_dtag, TString title, TString description, int nBins, double low, double high ) {
   smartBookHist(his_init, his_init, "i_" + title, description + "  [init.]", nBins, low, high);
   smartBookHist(his_init, his_init, "s_" + title, description + "  [seq.]", nBins, low, high);
...
private:
   ...
   typedef std::map<std::string, std::vector<TSmartHistValue> > THistValues;
...
public:
   TCutsManager(bool mc, bool good, TEvtRecDTag* evtRecDTag_ = 0, int charm_ = 0)
      : is_good(good)
      , has_mc(mc)
   ...
   void smartFillCutHistImpl(std::string name, double value, bool is_excbad = false) {
      initCut(name);
   ...
   void smartFillCutHist(std::string name, double value, bool is_excbad = false) {
      if (evtRecDTag) {
         smartFillCutHistImpl(TString::Format("%s_%d",name.c_str(),evtRecDTag->decayMode()).Data(), value, is_excbad);
      }
      ...
      if (charm != 0) {
         ...
         smartFillCutHistImpl((name + "_" + sign_str).data(), value, is_excbad);
      }
   ...
   void addBoundaries(std::string cut_name, TString boundaries) {
      if (!cuts_boundaries->Contains(cut_name.c_str())) {
         cuts_boundaries->Add(new TObjString(cut_name.c_str()), new TObjString(boundaries));
      }

   }


   bool rejectByCutIfLower(std::string cut_name, double value, double left) {
      if (value < left) {
   ...
   bool rejectByCutIfHigher(std::string cut_name, double value, double right) {
      if (value > right) {
         rejectByCut(cut_name);
   ...
   bool rejectByCutIfOutsideWindow(std::string cut_name, double value, double left, double right) {
   ...
   bool rejectByCutIfInsideWindow(std::string cut_name, double value, double left, double right) {
      if ( (value > left) && (value < right)) {
         rejectByCut(cut_name);
   ...
   void smartFillHistValues(const std::string & name, const std::vector<TSmartHistValue> & values) {
      for (std::vector<TSmartHistValue>::const_iterator value = values.begin();
           value != values.end(); ++value)
      {
         ...
         smartFillHist(name.c_str(), value->value, is_good, value->is_excbad, has_mc);
      }
   }
   ...
   void fillCutsStatistics() {
      ...
      for (TCutsList::const_iterator cut = cuts.begin();
           cut != cuts.end() && (cuts_applied.empty() || *cut != cuts_applied[0] );
           ++cut)
      {
   ...
   bool processCuts(bool fatal = false) {
      for (THistValues::const_iterator iter = hist_values.begin(); iter != hist_values.end(); ++iter) {
         const std::string & cut_name = iter->first;

       	if (!fatal) {
	         if (cuts_applied.empty() || ( cuts_applied.size() == 1 && cuts_applied[0] == cut_name )) {
	            //fill cut hist
...
static bool hasDecay(unsigned int eventTag, unsigned char top, unsigned char decay ) {
   ...
   if ((top == topDecay) && ( ( leftDecay == decay ) ||( rightDecay == decay) )) {
      return true;
   } else {
      return false;
   }
...
static bool selectDtagDecay(TEvtRecDTag* evtRecDTag, int decayMode, double deltaELow, double deltaEHigh, double mBCLow, double mBCHigh) {
   if ( (evtRecDTag->decayMode() == decayMode) ) {
      if ((evtRecDTag->deltaE() > deltaELow) && (evtRecDTag->deltaE() < deltaEHigh)) {
...
static bool fillDtagDecayHists(TEvtRecDTag* evtRecDTag, int decayMode, unsigned int eventTagDecay, int topEventTag, int decayEventTag, double deltaELow, double deltaEHigh, double mBCLow, double mBCHigh)
{
   bool selection_passed = selectDtagDecay(evtRecDTag, decayMode, deltaELow, deltaEHigh,  mBCLow, mBCHigh);

   if ( (evtRecDTag->decayMode() == decayMode) ) {
       if (hasDecay(eventTagDecay, topEventTag, decayEventTag )) {
         ((TH1*) hists_dtag_decay_good_mBC[decayMode])-> Fill( evtRecDTag->mBC() );
         ((TH1*) hists_dtag_decay_good_dE[decayMode]) -> Fill( evtRecDTag->deltaE() );

         ((TH2*) hists_dtag_decay_good_mBC_vs_dE[decayMode]) -> Fill( evtRecDTag->mBC(), evtRecDTag->deltaE());


         if (selection_passed) {
            ((TH1*) hists_dtag_decay_selected_good_mBC[decayMode]) -> Fill( evtRecDTag->mBC() );
         }
      } else {
         ((TH1*) hists_dtag_decay_bad_mBC[decayMode])-> Fill( evtRecDTag->mBC() );
         ((TH1*) hists_dtag_decay_bad_dE[decayMode]) -> Fill( evtRecDTag->deltaE() );

         ((TH2*) hists_dtag_decay_bad_mBC_vs_dE[decayMode]) -> Fill( evtRecDTag->mBC(), evtRecDTag->deltaE());

         if (selection_passed) {
            ((TH1*) hists_dtag_decay_selected_bad_mBC[decayMode]) -> Fill( evtRecDTag->mBC() );
         }
...
static bool is_electron(int index, ParticleID* pid, const  TObjArray * evtRecTrkCol, int run) {
   DstEvtRecTracks* dst_tracks = (DstEvtRecTracks*) evtRecTrkCol->At(index);
   ...
   if ( ( pid->probElectron() > 0.8 * (  pid->probPion() + pid->probKaon() + pid->probElectron())) &&
...
static unsigned int get_num_kaon_pion(const vector<Int_t>& tracks, const vector<Int_t> &  kp_tracks) {
...
static  bool isGoodPoca(BMdcKalTrack * mdcKalTrk, Hep3Vector& xorigin){
   ...
   if (  dz>= 10.0 ) return false;

   return true;
}


static bool isGoodChargedTrack(DstEvtRecTracks* tracks, Hep3Vector& xorigin) {
...
static double get_min_track_momentum(const TEvtRecDTag* evtRecDTag, const TObjArray* evtRecTrkCol, bool energy = false) {
   double min_p = 1E10;

   const vector<Int_t>& tracks = energy ? evtRecDTag->showers() : evtRecDTag->tracks();


   for (unsigned i = 0; i < tracks.size(); ++i) {
      DstEvtRecTracks* dstTracks =  (DstEvtRecTracks*) evtRecTrkCol->At(tracks[i]);
...
TMcParticle * getParticleByIdAndDecay(TMcEvent* m_TMcEvent, int particle_id, int decay) {
...
static void formatTag(char * tag_str, unsigned int eventTag_, TMcEvent* m_TMcEvent) {
    ...
    if ((hasDecay(eventTag_, 80, 8) && (m_EventTagSvc->getDecayCode(getParticleById(m_TMcEvent, 313)) == 1)) ||
        (hasDecay(eventTag_, 80, 12) && (m_EventTagSvc->getDecayCode(getParticleById(m_TMcEvent, 315)) == 1)) )
    {
...
const vector<const TMcParticle*> getDaughtersById(TMcEvent* m_TMcEvent, const TMcParticle * mother, int particle_id) {
   ...
   for (unsigned index = 0; index < daughters.size(); ++index) {
      const TMcParticle * daughter = m_TMcEvent->getMcParticle(daughters[index]);
...
const TMcParticle * getDaughterById(TMcEvent* m_TMcEvent, const TMcParticle * mother, int particle_id) {
   const vector<const TMcParticle*>& daughters = getDaughtersById(m_TMcEvent, mother, particle_id);
   if (!daughters.empty()) {
      return daughters[0];
   } else {
      return 0;
...
TAngularVars getAngularVars(HepLorentzVector electron4Momentum, HepLorentzVector missing4Momentum,
...

{
      ...
      HepLorentzVector d4Momentum = electron4Momentum + missing4Momentum + kaon4Momentum + pion4Momentum;
      ...
      Hep3Vector kpiNormal = kaon4Momentum.vect().cross(pion4Momentum.vect());
      ...
      Hep3Vector lnuNormal = electron4Momentum.vect().cross(missing4Momentum.vect());
...
void fillAngularVars(TNtuple * ntuple, const TAngularVars& vars, int tag_passed, bool passed, int decayMode, int charm) {
   //~ cout << "fillAngularVars: vars.mkpi=" << vars.mkpi << ", vars.q2=" << vars.q2 << " ,tag_passed=" << tag_passed << " , passed=" << passed << endl;
   ntuple->Fill(vars.q2, vars.mkpi, vars.theta_v, vars.theta_l, vars.chi, tag_passed, passed, vars.d_theta, vars.nu_theta, decayMode, charm);
}
...
{
   TAngularVars vars = getAngularVars(electron4Momentum, missing4Momentum, kaon4Momentum, pion4Momentum);
...
const TMcParticle* get_d_meson(TMcEvent* m_TMcEvent, int charm) {
   TMcParticle * psi_3770 =  getParticleByIdAndDecay(m_TMcEvent, 30443, 80);
   if (!psi_3770) return 0;

   const vector<const TMcParticle*>& d_mesons =  getDaughtersById(m_TMcEvent, psi_3770, 411); // get D mesons
...
bool getMcAngularVars(TMcEvent* m_TMcEvent, int charm, TAngularVars& vars) {
   ...
   int decay = m_EventTagSvc->getDecayCode(const_cast<TMcParticle*>(d_meson));
   ...
   } else if (decay == 12) {  // D+ => anti-K_2*0 e+  nu_e   , K_2*0 => K+  pi-
      const TMcParticle * k_star_2 = getDaughterById(m_TMcEvent, d_meson, 315);
   ...
   if (K && pi && nu && e) {
      vars = getAngularVars(getMcParticleP4(e), getMcParticleP4(nu), getMcParticleP4(K), getMcParticleP4(pi));
      return true;
   }
...
bool fillMcTracks(TMcEvent* m_TMcEvent, int charm, TNtuple * ntp, int tag_passed, bool passed, int decayMode) {
   TAngularVars vars;
...
bool fillMcMomenta(TMcEvent* m_TMcEvent, int charm, TNtuple * ntp, int tag_passed, bool passed) {
   // begin copypaste
   auto d_meson = get_d_meson(m_TMcEvent, charm);
   if (!d_meson) {
      return false;
...
TEvtRecDTag * selectDTag(const TObjArray* m_evtRecDTagCol, const TObjArray* evtRecTrkCol, int charm, int decayMode) {
      double min_distance = 1E10;
      TEvtRecDTag* evtRecDTag = 0;
      TIter evtRecDTagIter(m_evtRecDTagCol);
      while (TEvtRecDTag* curEvtRecDTag = (TEvtRecDTag*)  evtRecDTagIter.Next()) {
...
bool isGoodShower(RecEmcShower* emcTrk, const TObjArray* evtRecTrkCol, const TEvtRecEvent*  m_evtRecEvent, int exclude_track = -1) {
   if (!emcTrk) {
      return false;
   }

   cout << "check emcTrk " << emcTrk->trackId() <<" energy=" << emcTrk->energy() << " status=" << emcTrk->status() << endl;
   ...
   for (int charged_id = 0; charged_id < m_evtRecEvent->totalCharged(); ++charged_id) {
      if (charged_id != exclude_track) {
         DstEvtRecTracks* charged = (DstEvtRecTracks*) evtRecTrkCol->At(charged_id);
         if (! charged->isExtTrackValid())   continue;
         if (charged->extTrack()->emcVolumeNumber() == -1) continue;

         const Hep3Vector & chargedMomentum = charged->extTrack()->emcPosition();
   ...
   if  (   ( ( absCosTheta < 0.8 ) &&  ( energy > 25E-3) ) ||
           ( ( absCosTheta > 0.86 ) && ( absCosTheta < 0.92 ) && ( energy > 50E-3)  )   )
   {
      return true;
   } else {
...
bool get_good_d_to_k_pi_e_nu_mass(TMcEvent* m_TMcEvent, const TMcParticle * d_meson, double & mass) {
   int decay = m_EventTagSvc->getDecayCode(const_cast<TMcParticle*>(d_meson));
   ...
   if  (decay == 18) { //  K-  pi+  e+ nu_e
      ...
      const TMcParticle * k_star = getDaughterById(m_TMcEvent, d_meson, 313);
      if (m_EventTagSvc->getDecayCode(const_cast<TMcParticle*>(k_star)) == 1) { // K+  pi-
         mass = getMcParticleP4(k_star).m();
         return true;
      }
   } else if (decay == 12) {  // D+ => anti-K_2*0 e+  nu_e   , K_2*0 => K+  pi-
      const TMcParticle * k_star_2 = getDaughterById(m_TMcEvent, d_meson, 315);
      if (m_EventTagSvc->getDecayCode(const_cast<TMcParticle*>(k_star_2)) == 1) { // K+  pi-
...
bool is_good_k_pi_e_nu(TMcEvent* m_TMcEvent, int charm, double mass_low = -1E10, double mass_high = +1E10, double * out_mass = 0) {
   auto d_meson = get_d_meson(m_TMcEvent, charm);
   if (!d_meson) {
      return false;
   }
...
static multiset<long> to_pdg_set(const vector<string>& particle_list) {
...
static const multiset<long> get_final_state(TMcEvent* m_TMcEvent, const TMcParticle * parent, bool norm_sign = true) {
   ...
   static const multiset<long> ignore_particles = to_pdg_set(vector<string>({"gammaFSR"}));
   ...
   if (parent) {
      ...
      if (daughters.empty() || fs_particles.count(parent->getParticleID())) {
         // no further decay
         if (ignore_particles.count(parent->getParticleID()) == 0) { // not a FSR photon
         ...
         for (size_t index = 0; index < daughters.size(); ++index) {
            const TMcParticle * daughter = m_TMcEvent->getMcParticle(daughters[index]);
            auto daughter_final_state = get_final_state(m_TMcEvent, daughter, norm_sign);
            final_state.insert(daughter_final_state.begin(), daughter_final_state.end());
         }
      }
   }
...
int get_mc_tag_mode(TMcEvent* m_TMcEvent, int charm) { //charm of tag side
...
bool get_track_momentum(const DstEvtRecTracks * tracks, BMdcKalTrack::PidType kal_pid_type, Hep3Vector& p3) {
   assert(tracks);
...
DstEvtRecTracks* get_closest_track(const TObjArray* evtRecTrkCol, const vector<Int_t>& track_ids, const TMcParticle * ref_particle, double threshold = 1.0) {
   ...
   for (size_t index = 0; index < track_ids.size(); ++index) {
      DstEvtRecTracks* tracks = (DstEvtRecTracks*) evtRecTrkCol->At(track_ids[index]);
...
//-----------------------------------------------------------------------------
{
   ...
   his_aux.push_back( new TH1D("hDp_pMiss_x", "miss momentum", 200, -1, 1));
   his_aux.push_back( new TH1D("hDp_pMiss_y", "miss momentum", 200, -1, 1));
   his_aux.push_back( new TH1D("hDp_pMiss_z", "miss momentum", 200, -1, 1));
   his_aux.push_back( new TH1D("hDp_pMiss_e", "miss momentum", 200, -1, 1));



   //~ smartBookHist(his1_init, his1, his_dtag , "hDp_mBC", "Beam constrained mass of selected  tagged Dp", 100, 1.862, 1.878);
   smartBookHist(his1_init, his1, his_dtag , "hDp_dE", "delta energy  of selected  tagged Dp;#Delta E (GeV)", 750, -0.15, 0.15);
   smartBookHist(his1_init, his1, his_dtag , "hDp_mBC", "Beam constrained mass of selected  tagged Dp;m_{BC} (GeV)", 400, 1.83, 1.911);



   smartBookHist(his1_init, his1, his_dtag , "_hDp_dE_plus", "delta energy  of selected  tagged Dp", 750, -0.15, 0.15);
   smartBookHist(his1_init, his1, his_dtag , "_hDp_dE_minus", "delta energy  of selected  tagged Dp", 750, -0.15, 0.15);

   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_dE", "minimum dE of D0 tag;#Delta E (GeV)", 100, -0.1, 0.1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_dE_sideband", "minimum dE of D0 tag (mass sideband);#Delta E (GeV)", 100, -0.1, 0.1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_dE_sideband2", "minimum dE of D0 tag (mass sideband);#Delta E (GeV)", 100, -0.1, 0.1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_1_dE", "minimum dE of D0 tag 1;#Delta E (GeV)", 100, -0.1, 0.1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_3_dE", "minimum dE of D0 tag 3;#Delta E (GeV)", 100, -0.1, 0.1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_201_3_dE", "minimum dE of D0 tag;#Delta E (GeV)", 100, -0.1, 0.1);

   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_mode", "Decay mode of tagged D0", 510, 0,509);
   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_mode_sideband", "Decay mode of tagged D0 (mass sideband)", 510, 0,509);
   smartBookHist(his1_init, his1, his_dtag , "hDp_d0_mode_sideband2", "Decay mode of tagged D0 (mass sideband)", 510, 0,509);
   ...
   smartBookHist(his1_init, his1, his_dtag , "hDp_dE_mcgood", "delta energy  of selected  tagged Dp", 750, -0.15, 0.15);

   smartBookHist(his1_init, his_aux, his_dtag , "hDTag_DecayMode", "Decay mode of tagged D", 510, 0,509);

   smartBookHist(his1_init, his1, his_dtag , "hDp_mKPi", "Missing mass of K-#pi pair of %s events", 300, 0, 2);



   smartBookHist(his1_init, his1, his_dtag , "hDp_mMiss", "Missing mass squared of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 300, -0.1, 0.2);
   smartBookHist(his1_init, his1, his_dtag , "hDp_mMissBC", "BC Missing mass squared of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 300, -0.1, 0.2);

   smartBookHist(his1_init, his1, his_dtag , "hDp_uMiss", "Umiss = E - |p| %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 400, -0.2, 0.2);
   smartBookHist(his1_init, his1, his_dtag , "hDp_uMiss_cms", "CMS Umiss = E - |p| %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 400, -0.2, 0.2);
   smartBookHist(his1_init, his1, his_dtag , "hDp_uMiss_cms2", "CMS Umiss = E - |p| %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 400, -0.2, 0.2);
   smartBookHist(his1_init, his1, his_dtag , "hDp_uMiss_cms_bc", "CMS Umiss = E - |p| %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 400, -0.2, 0.2);

   smartBookHist(his1_init, his_aux, his_dtag , "hDp_mMissAlt", "Missing mass squared(using four-vectors) of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 100, -0.2, 0.2);
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_mMissInitial", "Missing mass squared(before any cuts) of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 100, -0.2, 0.2);


   smartBookHist(his1_init, his1, his_dtag , "hDp_goodOtherSide", "Good D+ candidate on the other side of %s ", 2, -.5, 1.5);

   smartBookHist(his1_init, his1, his_dtag , "hDp_nPions", "Number of %s pion candidates", 6, 0, 5);
   smartBookHist(his1_init, his1, his_dtag , "hDp_nKaons", "Number of %s kaon candidates", 6, 0, 5);

   smartBookHist(his1_init, his1, his_dtag , "hDp_n3tr_comb", "Number of %s 3 tracks combinations", 3, -0.5, 2.5);





   smartBookHist(his1_init, his_aux, his_dtag , "hDp_nCharged", "Number of %s charged candidates", 16, 0, 15);

   smartBookHist(his1_init, his1, his_dtag , "hDp_TotCharge", "Total charge of %s ", 20 , -10.5, 9.5);




   smartBookHist(his1_init, his_aux, his_dtag , "hDp_nShowers", "Number of %s chower candidates", 16, 0, 15);

   smartBookHist(his1_init, his1, his_dtag , "hDp_pMiss", "Missing momentum of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 100, -0.1, 0.9);
   smartBookHist(his1_init, his1, his_dtag , "hDp_EMiss", "Missing energy of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 100, -0.1, 0.9);


   smartBookHist(his1_init, his1, his_dtag , "hDp_missCosTheta", "Cosine of polar angle of missing momentum of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 50, 0, 1);
   ...
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_chiPrimVertex", "#chi^2 of primary vertex fit of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}.", 201,0,200 );
   smartBookHist(his1_init, his1, his_dtag , "hDp_mPiPi", "Invariant mass of %s K^{#pm} #pi^{#mp}  pair with mass of K^{#pm} candidate fixed at pi mass.", 160,0.2,1.4 );

   smartBookHist(his1_init, his1, his_dtag , "hDp_nPi0", "Number of #pi^{0} candidates with #chi^{2} < 20 (%s events)", 7, 0, 6);
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_pi0Chisq", "Distribution of #chi^{2} of  #pi^{0} candidates (%s events) ", 1000, 0, 1000);

   smartBookHist(his1_init, his1, his_dtag , "hDp_nGoodShowers", "N of good showers of  %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 7, -0.5, 6.5);
   smartBookHist(his1_init, his1, his_dtag , "hDp_showerE", "%s", 140, 0, 0.7);
   smartBookHist(his1_init, his1, his_dtag , "hDp_maxShowerE", "%s", 140, 0, 0.7);
   smartBookHist(his1_init, his1, his_dtag , "hDp_totalShowersE", "Total unassosiated shower energy of %s events", 100, 0, 1);


   smartBookHist(his1_init, his1, his_dtag , "hDp_costE", "cos(theta electron candidates of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} ", 100, -1, 1);
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_pxyK", "XY Momentum of K^{#pm} candidates of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} ", 100, 0, 1);
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_pxyE", "XY Momentum electron candidates of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} ", 100, 0, 1);
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_pxyPi", "XY Momentum of pi candidates of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} ", 100, 0, 1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_pE", "Momentum of electron candidates of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} ", 100, 0, 1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_pPi", "Momentum of pion candidates of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} ", 100, 0, 1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_pK", "Momentum of kaon candidates of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} ", 100, 0, 1);

   smartBookHist(his1_init, his1, his_dtag , "hDp_eE", "EMC Energy of electron candidates of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} ", 200, 0, 2);
   ...
   smartBookHist(his1_init, his1, his_dtag , "hDp_vi_MC_pMu", "initial MC Momentum of muon in  %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} (semimuonic excbad)", 100, 0, 1);

   smartBookHist(his1_init, his1, his_dtag , "hDp_MC_pPi", "MC Momentum of pion in semimuonic", 100, 0, 1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_vi_MC_pPi", "vi MC Momentum of pion in semimuonic", 100, 0, 1);
   ...
   smartBookHist(his1_init, his1, his_dtag , "hDp_mMiss_K_as_pi", "Missing mass of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} with Kaon mass replaced with Pion mass", 140, -0.1 , 0.6);
   smartBookHist(his1_init, his1, his_dtag , "hDp_mMiss_e_as_mu", "Missing mass of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e} with E mass replaced with Mu mass", 100, -0.08 , 0.08);
   ...
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_eRelProbE", "probE normalized to sum PID prob. %s tracks, electron candidate ", 800, 0 , 10000);
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_eRelProbMu", "probMu normalized to sum PID prob. %s tracks, electron candidate ", 1000, 0 ,0.1);
   smartBookHist(his1_init, his_aux, his_dtag , "hDp_eLogRelProbMu", " log probMu normalized to sum PID prob. %s tracks, electron candidate ", 1000, -100 ,100);

	smartBookHist(his1_init, his_aux, his_dtag , "hDp_nMucLayers", "N of layers in MUC with hits. %s tracks, electron candidate. (inclusive)", 15, -0.5, 14.5);
	smartBookHist(his1_init, his_aux, his_dtag , "hDp_Ntracks", "N of signal side tracks( %s events)", 15, -0.5, 14.5);

	smartBookHist(his1_init, his_aux, his_dtag , "hDp_goodTag", "Has good D+", 3, -0.5, 2.5);
	smartBookHist(his1_init, his_aux, his_dtag , "hDp_goodTag2", "Has good D+", 3, -0.5, 2.5);
	smartBookHist(his1_init, his_aux, his_dtag , "hDp_goodTagFlag", "Has good D+", 3, -0.5, 2.5);

	smartBookHist(his1_init, his_aux, his_dtag , "hDp_goodTagMode", "Mode of good D+", 501, -1.5, 499.5);
	smartBookHist(his1_init, his_aux, his_dtag , "hDp_goodTagMode2", "Mode of good D+", 501, -1.5, 499.5);
	smartBookHist(his1_init, his_aux, his_dtag , "hDp_goodTagMode3", "Mode of good D+", 501, -1.5, 499.5);
	smartBookHist(his1_init, his_aux, his_dtag , "hDp_goodMcTagMode", "Mode of good D+", 501, -1.5, 499.5);




   smartBookHist(his1_init, his1, his_dtag , "hDp_epEMC", "EMC energy deposit / momentum of electron candidates", 120, 0, 1.2);

   smartBookHist(his1_init, his1, his_dtag , "hDp_epEMC_sm_mupi", "EMC energy deposit / momentum of electron candidates, semileptonic mu as pi", 120, 0, 1.2);
   smartBookHist(his1_init, his1, his_dtag , "hDp_epEMC_sm_mue", "EMC energy deposit / momentum of electron candidates, semileptonic mu as e", 120, 0, 1.2);
   ...
   smartBookHist(his1_init, his1, his_dtag , "hDp_mMiss_mu", "Missing mass of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}. (bad is for semimuonic)", 200, -0.5, 0.5);

   smartBookHist(his1_init, his1, his_dtag , "hDp_mMiss_vxfit", "Missing mass squared (after VertexFit) of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 100, -0.1, 0.1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_mMiss_err", "Missing mass squared absolute error of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 100, 0, 0.04);

   smartBookHist(his1_init, his1, his_dtag , "hDp_mMiss_alt", "Missing mass squared (P4_cms - sum(P4_i) ) of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 100, -0.1, 0.1);
   smartBookHist(his1_init, his1, his_dtag , "hDp_mMiss_alt2", "Missing mass squared (using sum(p3_i) for dtag side ) of %s D^{#pm} #Rightarrow K^{#pm} #pi^{#mp} e^{#pm} #nu_{e}", 100, -0.1, 0.1);

   // FSR
   smartBookHist(his1_init, his1, his_dtag , "hDp_nFSR", "Number of FSR photons in %s ", 6, 0, 5);
   smartBookHist(his1_init, his1, his_dtag , "hDp_FSRE", "Total energy of FSR photons in %s", 100, 0, 0.5);

   smartBookHist(his1_init, his_aux, his_dtag , "hViSignalDecays", "very initial decay codes", 302, -1.5 , 300.5);
   smartBookHist(his1_init, his_aux, his_dtag , "hSignalDecays", "very initial decay codes", 302, -1.5 , 300.5);
   smartBookHist(his1_init, his_aux, his_dtag , "hGoodTagFlagSignalDecays", "very initial decay codes", 302, -1.5 , 300.5);
...
//~
   ...
   his_aux.push_back(new TH1D("hDp_mKPiTruthInitial", "True Missing mass of K-#pi pair of events (intitial histo)", 500, 0, 2));
   his_aux.push_back(new TH1D("hDp_mKPiTruthInitialGoodTag", "True Missing mass of K-#pi pair of events (intitial histo)", 500, 0, 2));


   his_aux.push_back(new TH1D("hDp_mKPiTruthVeryInitial", "True Missing mass of K-#pi pair of events (intitial histo)", 500, 0, 2));
   his_aux.push_back(new TH1D("hDp_mKPiTruth", "True Missing mass of K-#pi pair of events ", 500, 0, 2));
   his_aux.push_back(new TH1D("hDp_mKPiTruthKStar", "True Missing mass of K-#pi pair of K star events (intitial histo)", 500, 0, 2));

   his_aux.push_back(new TH1D("hDp_mBC_mctag_good", "Beam constrained mass of selected  tagged Dp (bad tags)", 400, 1.83, 1.911));
   his_aux.push_back(new TH1D("hDp_mBC_mctag_bad", "Beam constrained mass of selected  tagged Dp (bad tags)", 400, 1.83, 1.911));
   his_aux.push_back(new TH1D("hDp_mBC_tagmode_good", "Beam constrained mass of selected  tagged Dp (bad tags)", 400, 1.83, 1.911));
   his_aux.push_back(new TH1D("hDp_mBC_tagmode_bad", "Beam constrained mass of selected  tagged Dp (bad tags)", 400, 1.83, 1.911));
   his_aux.push_back(new TH1D("hDp_mBC_tagflag_good", "Beam constrained mass of selected  tagged Dp (bad tags)", 400, 1.83, 1.911));
   his_aux.push_back(new TH1D("hDp_mBC_tagflag_bad", "Beam constrained mass of selected  tagged Dp (bad tags)", 400, 1.83, 1.911));

   his_aux.push_back(new TH1D("hDp_4MDist", "4 momentum distance Dtag-MCtag", 600, -0.1, 1));
   his_aux.push_back(new TH1D("hDp_4MDist_mctag_good", "4 momentum distance Dtag-MCtag", 600, -0.1, 1));
   his_aux.push_back(new TH1D("hDp_4MDist_mctag_bad", "4 momentum distance Dtag-MCtag", 600, -0.1, 1));
   his_aux.push_back(new TH1D("hDp_4MDist_tagmode_good", "4 momentum distance Dtag-MCtag", 600, -0.1, 1));
   his_aux.push_back(new TH1D("hDp_4MDist_tagmode_bad", "4 momentum distance Dtag-MCtag", 600, -0.1, 1));
   ...
   his1.push_back( new TH2D("hDp_e_p_vs_cost", "p vs cos(t) for electron candidates;p_e (GeV);cos(#theta)", 11, 0.1, 1.2, 20, -1.0, 1.0));
   his1.push_back( new TH2D("hDp_e_p_vs_cost_good", "(good) p vs cos(t) for electron candidates;p_e (GeV);cos(#theta)", 11, 0.1, 1.2, 20, -1.0, 1.0));
   his1.push_back( new TH2D("s_hDp_e_p_vs_cost_good", "(good) p vs cos(t) for electron candidates;p_e (GeV);cos(#theta)", 11, 0.1, 1.2, 20, -1.0, 1.0));

   his1.push_back( new TH2D("hDp_e_p_vs_cost_we", "p vs cos(t) for electron candidates;p_e (GeV);cos(#theta)", 11, 0.1, 1.2, 20, -1.0, 1.0));
   his1.push_back( new TH2D("hDp_e_p_vs_cost_we_good", "(good) p vs cos(t) for electron candidates;p_e (GeV);cos(#theta)", 11, 0.1, 1.2, 20, -1.0, 1.0));
   his1.push_back( new TH2D("s_hDp_e_p_vs_cost_we_good", "(good) p vs cos(t) for electron candidates;p_e (GeV);cos(#theta)", 11, 0.1, 1.2, 20, -1.0, 1.0));
   ...
   for (int decay = 200; decay < 210; ++decay) {
      hists_dtag_decay_good_mBC[decay]   = new TH1D(TString::Format("hDp_decay_%d_good_mBC", decay), "Beam constrained mass of good kDptoKPiPi	(200)", 100, 1.83, 1.9);
      hists_dtag_decay_good_dE[decay]    = new TH1D(TString::Format("hDp_decay_%d_good_dE", decay), "delta energy  of good kDptoKPiPi	(200)", 100, -0.1, 0.1);
      hists_dtag_decay_good_Ntags[decay] = new TH1I(TString::Format("hDp_decay_%d_good_Ntags", decay), "Count of tags per good event DptoKPiPi	(200)", 11, 0, 10);

      hists_dtag_decay_bad_mBC[decay]   = new TH1D(TString::Format("hDp_decay_%d_bad_mBC", decay), "Beam constrained mass of bad kDptoKPiPi	", 100, 1.83, 1.9);
      hists_dtag_decay_bad_dE[decay]    = new TH1D(TString::Format("hDp_decay_%d_bad_dE", decay), "delta energy  of bad kDptoKPiPi", 100, -0.1, 0.1);


      hists_dtag_decay_selected_bad_mBC[decay] = new TH1D(TString::Format("hDp_decay_%d_selected_bad_mBC", decay), "Beam constrained mass of bad kDptoKPiPi passed the selection of deltaE", 100, 1.83, 1.9);
      hists_dtag_decay_selected_good_mBC[decay] = new TH1D(TString::Format("hDp_decay_%d_selected_good_mBC", decay), "Beam constrained mass of good kDptoKPiPi passed the selection of deltaE", 100, 1.83, 1.9);
      hists_dtag_decay_selected_mBC[decay] = new TH1D(TString::Format("hDp_decay_%d_selected_total_mBC", decay), "Beam constrained mass of all kDptoKPiPi passed the selection of deltaE", 100, 1.83, 1.9);

      hists_dtag_decay_good_mBC_vs_dE[decay] = new TH2D(TString::Format("hDp_decay_%d_good_mBC_vs_dE", decay), "Beam constrained mass of all good tracks versus deltaE", 100, 1.83, 1.9, 100, -0.1, 0.1);

      hists_dtag_decay_bad_mBC_vs_dE[decay] = new TH2D(TString::Format("hDp_decay_%d_bad_mBC_vs_dE", decay), "Beam constrained mass of bad tracks versus deltaE", 100, 1.83, 1.9, 100, -0.1, 0.1);



   }

   selector->RegInDir(hists_dtag_decay_good_mBC,"Semileptonic_Dtag_decays");
   selector->RegInDir(hists_dtag_decay_good_dE,"Semileptonic_Dtag_decays");
   selector->RegInDir(hists_dtag_decay_good_Ntags,"Semileptonic_Dtag_decays");
   selector->RegInDir(hists_dtag_decay_bad_mBC,"Semileptonic_Dtag_decays");
   selector->RegInDir(hists_dtag_decay_bad_dE,"Semileptonic_Dtag_decays");
   selector->RegInDir(hists_dtag_decay_selected_bad_mBC,"Semileptonic_Dtag_decays");
   selector->RegInDir(hists_dtag_decay_selected_good_mBC,"Semileptonic_Dtag_decays");
   selector->RegInDir(hists_dtag_decay_selected_mBC,"Semileptonic_Dtag_decays");

   selector->RegInDir(hists_dtag_decay_good_mBC_vs_dE,"Semileptonic_Dtag_decays");
   selector->RegInDir(hists_dtag_decay_bad_mBC_vs_dE,"Semileptonic_Dtag_decays");
   ...
   his_aux.push_back(new TH1I("hInitialBadTags", "hInitialBadTags tags", 11, 1,0));
   his_aux.push_back(new TH1I("hVeryInitialBadTags", "hVeryInitialBadTags tags", 11, 1,0));
   his_aux.push_back(new TH1I("hIntermediateAllTags", "All tags after 3tracks", 11, 1,0));


   his_aux.push_back(new TH1I("hRealEventSurvived", "Number of \"events\" survived in one real event", 13, -0.5, 12.5));
   his_aux.push_back(new TH1I("hCharmSurvived", "Number of \"events\" with same charm survived in one real event", 7, -0.5, 6.5));
   ...
   for (int decay = 200; decay < 206; ++decay) {
      his_aux.push_back(new TH1I(TString::Format("hGoodWrongTags_%d", decay), "GoodWrongTags", 11, 1,0));
      his_aux.push_back(new TH1I(TString::Format("hBadTags_%d", decay), "Bad tags for mode", 11, 1,0));
   }


   his_aux.push_back(new TH1I("hCuts_good", "Count of good events after each cut", 11, 1,0));
   his_aux.push_back(new TH1I("hCuts_bad", "Count of bad events after each cut", 11, 1,0));

   his_aux.push_back(new TH1I("hGoodMass_vi", "Is good event with good kpi mass, at the very begining", 2, -0.5, 1.5));
   his_aux.push_back(new TH1I("hGoodMass_hastag", "Is good event with good kpi mass, after some tag selection, before dE and mBC cuts", 2, -0.5, 1.5));
   his_aux.push_back(new TH1I("hGoodMass_mcgoodtag", "Is good event with good kpi mass, after some tag selection, before dE and mBC cuts provided tag was good (MC)", 2, -0.5, 1.5));
   his_aux.push_back(new TH1I("hGoodMass_mcgoodtag2", "Is good event with good kpi mass, after some tag selection, before dE and mBC cuts provided tag was good (MC)", 2, -0.5, 1.5));
   his_aux.push_back(new TH1I("hGoodMass_good_tag_flag", "Is good event with good kpi mass, after some tag selection, before dE and mBC cuts provided tag was good (MC)", 2, -0.5, 1.5));
   his_aux.push_back(new TH1I("hGoodMass_d", "Is good event with good kpi mass, after d selection", 2, -0.5, 1.5));
   ...
   hist("hBadTags")->Fill("blah",0.0); //one MUST keep this for correct merging!
   hist("hAllTags")->Fill("blah",0.0); //one MUST keep this for correct merging!
   hist("hInitialAllTags")->Fill("blah",0.0); //one MUST keep this for correct merging!
   hist("hInitialBadTags")->Fill("blah",0.0); //one MUST keep this for correct merging!
   hist("hVeryInitialBadTags")->Fill("blah",0.0); //one MUST keep this for correct merging!
   hist("hIntermediateAllTags")->Fill("blah",0.0); //one MUST keep this for correct merging!


   hist("hAux")->Fill("blah",0.0); //one MUST keep this for correct merging!
   hist("hBadTags_e")->Fill("blah",0.0); //one MUST keep this for correct merging!
   hist("hCuts_good")->Fill("blah",0.0); //one MUST keep this for correct merging!
   hist("hCuts_bad")->Fill("blah",0.0); //one MUST keep this for correct merging!
   ...
   ntp_features = new TNtuple("ntp_features","Features ntuple","good:ep:pmiss:muclayers:etotal:mmiss_china:mmiss");


   VecObj features_ntuples(1, ntp_features);
   selector->RegInDir(features_ntuples,"Semileptonic_Features");

   char angular_vars[] = "q2:mkpi:theta_v:theta_l:chi:tag_passed:passed:d_theta:nu_theta:tag_mode:charm";
   ntp_angular_selected = new TNtuple("ntp_angular_selected","Angular data of the survived itemsntuple",angular_vars);
   ntp_angular_mc = new TNtuple("ntp_angular_mc","Angular data of the MC truth information",angular_vars);


   char momenta_vars[] = "l_px:l_py:l_pz:l_pe:nu_px:nu_py:nu_pz:nu_pe:k_px:k_py:k_pz:k_pe:pi_px:pi_py:pi_pz:pi_pe:tag_passed:passed:is_good";

   ntp_momenta_mc = new TNtuple("ntp_momenta_mc","Momenta data of the MC truth information",momenta_vars);

   ntp_angular_selected_wmc = new TNtuple("ntp_angular_selected_wmc", "", "q2:mkpi:theta_v:theta_l:chi:mc_q2:mc_mkpi:mc_theta_v:mc_theta_l:mc_chi:tag_passed:good_mc:tag_mode");
   ...
   ntp_electrons = new TNtuple("ntp_electrons", "ntp_electrons", "e_emc:p:cost:charge:evt_tot_emc:evt_dphi:pid_prob_e:pid_prob_sum:passed"  );
   VecObj e_ntuples;
...
//-----------------------------------------------------------------------------
{
  ...
  Hep3Vector xorigin = getVertexOrigin(run);
   ...
   unsigned int eventTagDecay =  (m_EventTagSvc->getEventTag()  & 0xFFFFFF00) >> 8;
   ...
   good_oldalgo |= hasDecay(eventTagDecay, 80, 8) && (m_EventTagSvc->getDecayCode(getParticleById(m_TMcEvent, 313)) == 1); // K+  pi-
   good_oldalgo |= hasDecay(eventTagDecay, 80, 12) && (m_EventTagSvc->getDecayCode(getParticleById(m_TMcEvent, 315)) == 1); // D+ => anti-K_2*0 e+  nu_e   , K_2*0 => K+  pi-

   if (!m_TEvtRecObject) {
      // rtraw file, nothing to do...
      for (int charm = -1;  charm <= 1; charm+=2) {
         for (int decayMode = 200; decayMode < 206; ++decayMode) {
            fillMcTracks(m_TMcEvent, charm, ntp_angular_mc, false, false, decayMode);
         }
      }
   ...
   const TObjArray*  m_evtRecDTagCol= m_TEvtRecObject->getEvtRecDTagCol();
   ...
   for (int charm = -1;  charm <= 1; charm+=2) {
      ...
      bool has_good_kpi_mass = is_good_k_pi_e_nu(m_TMcEvent, charm, 0.8, 1.0);
      ...
      bool good_by_final_state = (get_final_state(m_TMcEvent, signal_d_meson) == to_pdg_set(vector<string>({"K+","pi+", "e-", "nu_e"})));

      if (good != good_by_final_state) {
         cout << "good_by_final_state(" << good_by_final_state << ") != good (" << good << ") !" <<endl;
      ...
      if (signal_d_meson) {
         signal_decay_code = m_EventTagSvc->getDecayCode(const_cast<TMcParticle*>(signal_d_meson));
      }
      ...
      for (int decayMode = 200; decayMode < 206; ++decayMode) {
         ...
         TEvtRecDTag* evtRecDTag = selectDTag(m_evtRecDTagCol, evtRecTrkCol, -1 * charm, decayMode);


         bool is_semimuonic =  (get_final_state(m_TMcEvent, signal_d_meson) == to_pdg_set(vector<string>({"K+","pi+", "mu-", "nu_mu"})));



         TCutsManager cutsManager(!m_TMcEvent->getMcParticleCol()->IsEmpty(), good, evtRecDTag, charm);
         ...
         bool mc_is_good_tag =  ( (mc_tag_mode >= 200) && (mc_tag_mode <= 205));
         ...
         if (!evtRecDTag) {
            ...
            fillMcTracks(m_TMcEvent, charm, ntp_angular_mc, tag_passed, false, decayMode);
         ...
         if (is_semimuonic && signal_d_meson) {
            //find pion
            const TMcParticle * pi = 0;
            int decay = m_EventTagSvc->getDecayCode(const_cast<TMcParticle*>(signal_d_meson));

            if  (decay == 42) { //     K-  pi+  mu+ nu_e
               pi = getDaughterById(m_TMcEvent, signal_d_meson, 211);
            } else if (decay == 32 ) { //   anti-K*0  mu+  nu_mu
               const TMcParticle * k_star = getDaughterById(m_TMcEvent, signal_d_meson, 313);
               pi = getDaughterById(m_TMcEvent, k_star, 211);
            } else if (decay == 36) {  // 36   anti-K_2*0 mu+  nu_mu
               const TMcParticle * k_star_2 = getDaughterById(m_TMcEvent, signal_d_meson, 315);
               pi = getDaughterById(m_TMcEvent, k_star_2, 211);
            }
            ...
            if (!pi) {
               ...
               cutsManager.smartFillCutHist( "hDp_vi_MC_pPi", mc_pion_4momentum.vect().mag());
               cutsManager.smartFillCutHist( "hDp_vi_MC_pMu", mc_muon_4momentum.vect().mag());
         ...
         if (mc_is_good_tag && !good_tag_mode) {
            cout << "wrong tag mode: mc " << mc_tag_mode  << "!=" << evtRecDTag->decayMode() << endl;
         ...
         if (good_tag_mode ) {
            cutsManager.smartFillCutHist("hDp_goodTagMode", evtRecDTag->decayMode());
         } else {
         ...
         cutsManager.smartFillCutHist("hDTag_DecayMode", evtRecDTag->decayMode());

         if ( (evtRecDTag->decayMode()  < 200) || (evtRecDTag->decayMode()  > 205) ) {
         ...
         cutsManager.rejectByCutIfOutsideWindow(TString::Format("hDp_dE_%d", evtRecDTag->decayMode()).Data(), evtRecDTag->deltaE(), dE_low, dE_high);
         ...
         cutsManager.rejectByCutIfOutsideWindow("hDp_mBC", evtRecDTag->mBC(), 1.863, 1.877);




         cutsManager.smartFillCutHist("hDp_goodTagMode2", good_tag_mode ? evtRecDTag->decayMode() : -1);
         cutsManager.smartFillCutHist("hDp_goodTagMode3", good_tag_flag ? evtRecDTag->decayMode() : -1);



         cutsManager.smartFillCutHist("hDp_goodMcTagMode", mc_is_good_tag ? mc_tag_mode : -1);
         ...
         cutsManager.smartFillCutHist("hGoodTagFlagSignalDecays",good_tag_flag ? signal_decay_code : -1);
         ...
         for (unsigned track_i = 0; track_i < otrk.size(); ++track_i) {
            DstEvtRecTracks* tracks =  (DstEvtRecTracks*) evtRecTrkCol->At(otrk[track_i]);
            if (!isGoodChargedTrack(tracks, xorigin)) continue;
         ...
         for (int tmpi = 0; tmpi < 3; ++tmpi) {
            DstEvtRecTracks* tracks =  (DstEvtRecTracks*) evtRecTrkCol->At(signal_tracks[tmpi]);
         ...
         for (unsigned pion_id = 0; pion_id < signal_tracks.size() ; ++pion_id) {
            for (unsigned kaon_id = 0; kaon_id < signal_tracks.size() ; ++kaon_id) {
               if (pion_id != kaon_id)
                  for (unsigned electron_id = 0; electron_id < signal_tracks.size() ; ++electron_id)
                     if ( (electron_id != kaon_id) && (electron_id != pion_id)) {

                        DstEvtRecTracks * _kaon_tracks = (DstEvtRecTracks*) evtRecTrkCol->At(signal_tracks[kaon_id]);
                        DstEvtRecTracks * _pion_tracks = (DstEvtRecTracks*) evtRecTrkCol->At(signal_tracks[pion_id]);
                        DstEvtRecTracks * _e_tracks = (DstEvtRecTracks*) evtRecTrkCol->At(signal_tracks[electron_id]);
                        ...
                        if (   is_pion_dtag( signal_tracks[pion_id], evtRecDTag) &&
                             ...
                             ( _kaon_tracks->mdcTrack()->charge() == -_pion_tracks->mdcTrack()->charge()) &&
                             ( _e_tracks->mdcTrack()->charge() == _pion_tracks->mdcTrack()->charge())
         ...
         cutsManager.smartFillCutHist( "hDp_n3tr_comb",  n_3tracks_combinations);
         cutsManager.rejectByCutIfHigher( "hDp_n3tr_comb",  n_3tracks_combinations, 1.01);
         ...
         HepLorentzVector kaon4Momentum = kaon_tracks->mdcKalTrack()->p4(MASS_KAON);
         HepLorentzVector pion4Momentum = pion_tracks->mdcKalTrack()->p4(MASS_PION);
         double mKPi = (pion4Momentum + kaon4Momentum).m();
         if (good) {
            double diff  = (mKPi_true - mKPi) * 100.0 / mKPi;
            cout << tag_str << " mKPi_true: " << mKPi_true << ", mKPi: " << mKPi << ", diff: " <<  diff <<  "%" << endl;
            if (diff > 10) {
               cout << "big diff" << endl;
            }
         ...
         while (TEvtRecDTag* otherEvtRecDTag = (TEvtRecDTag*)  otherEvtRecDTagIter.Next()) {
            ...
            if (goodIntersection) {
               ...
               goodOtherTag |= selectDtagDecay(otherEvtRecDTag, 200, -0.015, 0.015, 1.866, 1.875);
            }
         }
      ...
      // FSR recovery tests
         cout << "e_tracks->mdcKalTrack()->p3(): " << e_tracks->mdcKalTrack()->p3() << endl;
         cout << "e kal th: " << e_tracks->mdcKalTrack()->theta() << " phi: " << e_tracks->mdcKalTrack()->phi() << endl;
         cout << "e kal p3 th: " << e_tracks->mdcKalTrack()->p3().theta() << " phi: " << e_tracks->mdcKalTrack()->p3().phi() << endl;
         cout << "e mdc th: " << e_tracks->mdcTrack()->theta() << " phi: " << e_tracks->mdcTrack()->phi() << endl;

         if (e_tracks->isEmcShowerValid()) {
            cout << "e shower th: " << e_tracks->emcShower()->theta() << " phi: " << e_tracks->emcShower()->phi() << endl;
            if (e_tracks->isExtTrackValid()) {
               cout << "e emc ext th: " << e_tracks->extTrack()->emcPosition().theta() << " phi: " << e_tracks->extTrack()->emcPosition().phi() << endl;
            }
         }
         ...
         for (unsigned shower_id = 0; shower_id < oshw.size(); ++shower_id) {
            DstEvtRecTracks* shower_tracks = (DstEvtRecTracks*) evtRecTrkCol->At(oshw[shower_id]);
            RecEmcShower *emcTrk = shower_tracks->emcShower();

            if (!isGoodShower(emcTrk, evtRecTrkCol, m_evtRecEvent, /*exclude_track=*/e_tracks->trackId())) {
               continue;
            }

            Hep3Vector showerVector = getShowerVector(emcTrk);

            double initial_angle =  showerVector.angle(e_tracks->mdcKalTrack()->p3());
            cout << "shower id: " << oshw[shower_id] << " angle to initial electron: " <<  initial_angle / TMath::DegToRad() << " degrees" << endl;
            if (initial_angle < TMath::DegToRad() * 5.0 ) {
               // found fsr photon
               cout << "found FSR photon: " << oshw[shower_id] << ", energy: " << emcTrk->energy() * 1E3<< " MeV"<< endl;
               cout << "FSR shower th: " << emcTrk->theta() << " phi: " << emcTrk->phi() << endl;
      ...
      // end of FSR tests
         ...
         bool with_electron = good ||  hasDecay(eventTagDecay, 80, 9 )  ||  hasDecay(eventTagDecay, 80, 10 )  ||  hasDecay(eventTagDecay, 80, 11) ||  hasDecay(eventTagDecay, 80, 12)                       ||  hasDecay(eventTagDecay, 80, 13) ||  hasDecay(eventTagDecay, 80, 14) ||  hasDecay(eventTagDecay, 80, 15) ||  hasDecay(eventTagDecay, 80, 16) ||  hasDecay(eventTagDecay, 80, 17) ||  hasDecay(eventTagDecay, 80, 18) ||  hasDecay(eventTagDecay, 80, 19) ||  hasDecay(eventTagDecay, 80, 20) ;
   ...
   //~
         ...
         cutsManager.smartFillCutHist(  "hDp_pPi", pion_tracks->mdcKalTrack()-> p());
         cutsManager.smartFillCutHist(  "hDp_pK", kaon_tracks->mdcKalTrack()-> p());
         ...
         kmfit->init();
         Hep3Vector taggedDMomentum(evtRecDTag->px(), evtRecDTag->py(), evtRecDTag->pz());
         ...
         cout << "eventTagDecay: " << eventTagDecay << " text:   " << tag_str << endl;
         ...
         expected4Momentum.setVectM(initialMomentum -taggedDMomentum, D_PLUS_MASS);
         ...
         cout << "missing mass: " <<  (ecms -  taggedD4Momentum - observed4Momentum).m2() << endl;
         cout << "missing mass2: " <<  (expected4Momentum - observed4Momentum).m2() << endl;
         ...
         expectedD4Momentum_cms.setVectM(-taggedD4Momentum_cms.vect(), D_PLUS_MASS );
         ...
         expectedD4Momentum_cms2.setVectM(-taggedD4Momentum_cms2.vect(), D_PLUS_MASS );




         HepLorentzVector missing4MomentumBC_cms = expectedD4MomentumBC_cms - observed4Momentum_cms;
         HepLorentzVector missing4Momentum_cms = expectedD4Momentum_cms - observed4Momentum_cms;
         HepLorentzVector missing4Momentum_cms2 = expectedD4Momentum_cms2 - observed4Momentum_cms;


         cout << "expectedD4MomentumBC_cms: " << expectedD4MomentumBC_cms << endl;
         ...
         double missingEnergy_cms = evtRecDTag->beamE() - observed4Momentum_cms.e();
         ...
         cout << "missingEnergy_cms rel delta: " << (missingEnergy_cms-missingEnergy)/missingEnergy << endl;
         ...
         Hep3Vector missingMomentum = (expected4Momentum.vect() - observedMomentum);

         HepLorentzVector missing4Momentum = (expected4Momentum - observed4Momentum);
         ...
         smartFillHist("hDp_mMissInitial",  (expected4Momentum - observed4Momentum).m2(), good);
         ...
         cutsManager.rejectByCutIfLower("hDp_EMiss", missingEnergy_cms , 0.04); // this suppresses sigificant part of  K*_20 decay channel!
      ...
      //~
         ...
         // ========== MUC hits
         int nMucLayers =  e_tracks->isMucTrackValid() ? e_tracks->mucTrack()->numLayers() : 0;

         cutsManager.smartFillCutHist( "hDp_nMucLayers", nMucLayers, is_semimuonic);
         ...
         double massKPiAsPiPi = (kaon_tracks->mdcTrack()->p4(MASS_PION) + pion_tracks->mdcTrack()->p4(MASS_PION)).m();
         ...
         cutsManager.smartFillCutHist( "hDp_nPions",  get_num_kaon_pion(evtRecDTag->otherTracks(),evtRecDTag->pionId()));
         ...
         cutsManager.smartFillCutHist( "hDp_nKaons",  get_num_kaon_pion(evtRecDTag->otherTracks(),evtRecDTag->kaonId()));
         ...
         cutsManager.smartFillCutHist( "hDp_nCharged",  evtRecDTag->otherTracks().size());
         cutsManager.smartFillCutHist( "hDp_nShowers",  evtRecDTag->otherShowers().size(), hasPi0(m_TMcEvent));
         ...
         cutsManager.smartFillCutHist( "hDp_nGoodShowers", nGoodShowers,  hasPi0(m_TMcEvent));


         cutsManager.smartFillCutHist( "hDp_maxShowerE", maxShowerE,  hasPi0(m_TMcEvent));
         cutsManager.rejectByCutIfHigher("hDp_maxShowerE", maxShowerE , 0.250);



         cutsManager.smartFillCutHist( "hDp_totalShowersE", totalShowersE,  hasPi0(m_TMcEvent));
         ...
         double mMiss_K_as_pi = (expected4Momentum - observed4Momentum_KaonAsPion).m2();
         ...
         double mMiss_e_as_mu = (expected4Momentum - observed4Momentum_e_as_mu).m2();
              ...
              (evtRecDTag->decayMode() == 204) )
         {
            ...
            while (TEvtRecDTag* d0EvtRecDTag = (TEvtRecDTag*)  d0EvtRecDTagIter.Next()) {
               if (d0EvtRecDTag->type() != 1) continue;
               //~ if (d0EvtRecDTag->charm() != evtRecDTag->charm()) continue;
               if (d0EvtRecDTag->decayMode() > 126) continue;
         ...
         cutsManager.smartFillCutHist( "hDp_d0_mode_sideband", d0_mode_sideband, hasDecay(eventTagDecay, 81, 8 ) );
         cutsManager.smartFillCutHist( "hDp_d0_dE_sideband", d0_dE_sideband, hasDecay(eventTagDecay, 81, 8 ) );
         cutsManager.smartFillCutHist( "hDp_d0_mode_sideband2", d0_mode_sideband2, hasDecay(eventTagDecay, 81, 8 ) );
         cutsManager.smartFillCutHist( "hDp_d0_dE_sideband2", d0_dE_sideband2, hasDecay(eventTagDecay, 81, 8 ) );

         cutsManager.smartFillCutHist( "hDp_d0_mode", d0_mode, hasDecay(eventTagDecay, 81, 8 ) );
         cutsManager.smartFillCutHist( "hDp_d0_dE", d0_dE, hasDecay(eventTagDecay, 81, 8 ) );


         cutsManager.rejectByCutIfInsideWindow("hDp_d0_dE", d0_dE, -0.01, 0.01);
         ...
         double uMiss = (expected4Momentum - observed4Momentum).e() - (expected4Momentum - observed4Momentum).vect().mag();
         double uMiss_cms_bc = missing4MomentumBC_cms.e() - missing4MomentumBC_cms.vect().mag();
         double uMiss_cms = missing4Momentum_cms.e() - missing4Momentum_cms.vect().mag();
         double uMiss_cms2 = missing4Momentum_cms2.e() - missing4Momentum_cms2.vect().mag();


         cutsManager.smartFillCutHist( "hDp_mMissBC", missing4MomentumBC_cms.m2());

         cutsManager.smartFillCutHist( "hDp_mMiss", mMiss, hasDecay(eventTagDecay, 80, 11 ) );
         //~ cutsManager.rejectByCutIfOutsideWindow("hDp_mMiss", mMiss , -0.04, 0.04);


         cout << "U_miss_cms_BC= " << uMiss_cms << endl;
         if (cutsManager.survived()) {
            cout << "run= " << run << " event= " << event << " U_miss=" << uMiss << endl;
         }
         ...
         cutsManager.rejectByCutIfOutsideWindow("hDp_uMiss_cms_bc", uMiss_cms_bc , -0.04, 0.04);


         //~ cout << "ep_ratio: " << ep_ratio << endl;
         ntp_features->Fill(good, ep_ratio, missingMomentum.mag(), nMucLayers, totalShowersE,mMiss_K_as_pi,mMiss);
         ...
         if (cutsManager.survived()) {
            ...
            fillAngularVars(ntp_angular_selected, electron4Momentum, missing4Momentum, kaon4Momentum, pion4Momentum, tag_passed, true, decayMode, charm);
            ...
            TAngularVars rec_vars = getAngularVars(electron4Momentum, missing4Momentum, kaon4Momentum, pion4Momentum);
            ...
            cout << "rec_vars: q2= " << rec_vars.q2 << " , mkpi= " << rec_vars.mkpi << " , theta_v= " << rec_vars.theta_v << " ,theta_l=" << rec_vars.theta_l << " ,chi= " << rec_vars.chi << endl;

            TAngularVars mc_vars;
            bool good_mc = getMcAngularVars(m_TMcEvent, charm, mc_vars);

            ntp_angular_selected_wmc->Fill(rec_vars.q2, rec_vars.mkpi, rec_vars.theta_v, rec_vars.theta_l, rec_vars.chi,
                                           mc_vars.q2, mc_vars.mkpi, mc_vars.theta_v, mc_vars.theta_l, mc_vars.chi,
         ...
         if (cutsManager.survived()) {
            ...
            smartFillHist("hDp_missCosTheta",  TMath::Abs(missingMomentum.cosTheta()), good);
            ...
            if (! good) {
                  ...
                  hist(TString::Format("hBadTags_%d", evtRecDTag->decayMode()))->Fill(tag_str, 1.0);
            }

            if (good && !good_tag_mode) {
               cout << "good with bad tag" << endl;
               hist(TString::Format("hGoodWrongTags_%d", evtRecDTag->decayMode()))->Fill(tag_str, 1.0);
            }
            ...
            for (unsigned id = 0; id < dtag_tracks.size(); ++id) {
               RecMdcKalTrack* kalTrk = ((DstEvtRecTracks*) evtRecTrkCol->At(dtag_tracks[id]))->mdcKalTrack();
               ...
               WTrackParameter wvTrk = WTrackParameter(mass, kalTrk->getZHelix(), kalTrk->getZError());
            ...
            vtxfit->AddTrack(dtag_tracks.size(), WTrackParameter(MASS_ELECTRON, e_tracks->mdcKalTrack()->getZHelix(), e_tracks->mdcKalTrack()->getZError()));
            vtxfit->AddTrack(dtag_tracks.size() + 1, WTrackParameter(MASS_PION, pion_tracks->mdcKalTrack()->getZHelix(), pion_tracks->mdcKalTrack()->getZError()));
            vtxfit->AddTrack(dtag_tracks.size() + 2, WTrackParameter(MASS_KAON, kaon_tracks->mdcKalTrack()->getZHelix(), kaon_tracks->mdcKalTrack()->getZError()));
            ...
            expected4Momentum_alt.setVectM(ecms.vect() -tagMomentum_alt, D_PLUS_MASS);
...
//~
         ...
         fillMcTracks(m_TMcEvent, charm, ntp_angular_mc, tag_passed, event_passed, decayMode);
