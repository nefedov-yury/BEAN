...
//-----------------------------------------------------------------------------
{
   ...
   ntp_electrons = new TNtuple("ntp_electrons", "ntp_electrons", "e_emc:p:cost:charge:evt_tot_emc:evt_dphi:pid_prob_e:pid_prob_sum:evt_phi_low:evt_phi_high:evt_theta_low:evt_theta_high"  );
...
//-----------------------------------------------------------------------------
{
  ...
  Hep3Vector xorigin = getVertexOrigin(run);
   ...
   if (!(m_evtRecEvent->totalCharged() == 2)) { return false; };
   ...
   for (int i = 0; i < 2 ; ++ i) {
      ...
      if ( !track_1->isMdcKalTrackValid()) {return false;}
      track->mdcKalTrack()->setPidType(BMdcKalTrack::electron);

      if (track->mdcKalTrack()->charge() == 0) { return false; }

      if (!isGoodPoca(track->mdcKalTrack(), xorigin)) { return false;}

      if ( TMath::Abs(TMath::Cos(track->mdcKalTrack()->theta())) > 0.93) { return false;}
   }


   if (track_1->mdcKalTrack()->charge() != -1 * track_2->mdcKalTrack()->charge()) { return false;}

   HepVector poca_1 = getPoca(track_1->mdcKalTrack(), xorigin);
   HepVector poca_2 = getPoca(track_1->mdcKalTrack(), xorigin);
   if (TMath::Abs(poca_1[3] - poca_2[3]) >= 2.0) { return false;};

   double delta_phi = TMath::Pi() - TMath::Abs(track_1->mdcKalTrack()->phi() - track_2->mdcKalTrack()->phi());
   //~ double delta_phi = track_1->mdcKalTrack()->phi() - track_2->mdcKalTrack()->phi();
   double delta_phi2 = track_1->mdcKalTrack()->phi() + track_2->mdcKalTrack()->phi();
   ...
   for (unsigned i = 0; i < m_evtRecEvent->totalTracks(); ++i) {
      DstEvtRecTracks* track = (DstEvtRecTracks*) evtRecTrkCol->At(i);
   ...
   if (max_gamma_energy < 30E-3) { return false;}
   ...
   if (!track_high->isEmcShowerValid()) { return false;}
   ...
   if ( (high_ep_ratio > 1.05) || (high_ep_ratio < 0.85)) { return false;}

   if (track_low->mdcKalTrack()->p() > 1.3) { return false;}
