...
HepLorentzVector calc_miss_4momenum(TEvtRecDTag* evtRecDTag, HepLorentzVector observed4Momentum ) {
...
//~
  ...
  //~ }
   ...
   his1.push_back( new TH1D("h_ncharged", "h_ncharged", 10,-0.5,9.5  ));
   his1.push_back( new TH1D("h_200_200_dE", "delta energy  of selected  tagged Dp;#Delta E (GeV)", 750, -0.15, 0.15));
   his1.push_back( new TH1D("h_200_200_mBC", "Beam constrained mass of selected  tagged Dp;m_{BC} (GeV)", 400, 1.83, 1.911));
   his1.push_back( new TH1D("h_200_200_maxShowerE", "h_200_200_maxShowerE", 100, 0, 0.5));

   ntp_double = new TNtuple("ntp_double", "ntp_double", "mode_1:mode_2:dE_1:dE_2:mBC_1:mBC_2:max_shower_e:evt_tag:run:u_miss:p_miss");
...
selectDoubleDTag(const TObjArray* m_evtRecDTagCol, const TObjArray* evtRecTrkCol, int decayMode1, int decayMode2 ) {
      vector<pair<TEvtRecDTag*, TEvtRecDTag*> > result ;


      TIter evtRecDTag1Iter(m_evtRecDTagCol);

      while (TEvtRecDTag* curEvtRecDTag1 = (TEvtRecDTag*)  evtRecDTag1Iter.Next()) {
         if (curEvtRecDTag1->type() != 1) { continue; }// require PID

         if (curEvtRecDTag1->decayMode() != decayMode1)  { continue; }
           // found first

         TIter evtRecDTag2Iter = evtRecDTag1Iter;
         while (TEvtRecDTag* curEvtRecDTag2 = (TEvtRecDTag*)  evtRecDTag2Iter.Next()) {
            if (curEvtRecDTag2->type() != 1) { continue; }// require PID
            if (curEvtRecDTag2->charm() ==  curEvtRecDTag1->charm()) { continue; }// skip same charm

            if (curEvtRecDTag2->decayMode() != decayMode2)  {continue; } //skip wrong mode
               // compare tracks

            if (std::set<Int_t>(curEvtRecDTag1->tracks().begin(), curEvtRecDTag1->tracks().end()) !=
                std::set<Int_t>(curEvtRecDTag2->otherTracks().begin(), curEvtRecDTag2->otherTracks().end()))
            {
               continue;
            }
            ...
            // no common photons
            if (!is_disjoint(std::set<Int_t>(curEvtRecDTag1->showers().begin(), curEvtRecDTag1->showers().end()),
                             std::set<Int_t>(curEvtRecDTag2->showers().begin(), curEvtRecDTag2->showers().end())))
            {
            ...
            cout << "  dtag1: mode=" << curEvtRecDTag1->decayMode() << " charm=" << curEvtRecDTag1->charm() << endl;
            cout << "         mBC=" << curEvtRecDTag1->mBC() << " deltaE=" << curEvtRecDTag1->deltaE() << endl;

            cout << "  dtag2: mode=" << curEvtRecDTag2->decayMode() << " charm=" << curEvtRecDTag2->charm() << endl;
            cout << "         mBC=" << curEvtRecDTag2->mBC() << " deltaE=" << curEvtRecDTag2->deltaE() << endl;
...
//-----------------------------------------------------------------------------
{
   ...
   auto double_tags_200_201 = selectDoubleDTag(m_evtRecDTagCol, evtRecTrkCol, 200, 201);
   auto double_tags_200_200 = selectDoubleDTag(m_evtRecDTagCol, evtRecTrkCol, 200, 200);

   auto double_tags = double_tags_200_200;
   double_tags.insert( double_tags.end(), double_tags_200_201.begin(), double_tags_200_201.end() );
   ...
   for (size_t i = 0; i < double_tags.size(); ++i)
   {
      ...
      double maxShowerE = 0;
      for (unsigned shower_id = m_evtRecEvent->totalCharged(); shower_id < m_evtRecEvent->totalTracks(); ++shower_id) {
         //check this shower doesn't belong to any of D
         if (find(it.first->showers().begin(), it.first->showers().end(), shower_id) != it.first->showers().end()) { continue; }
         if (find(it.second->showers().begin(), it.second->showers().end(), shower_id) != it.second->showers().end()) { continue; }



         DstEvtRecTracks* track = (DstEvtRecTracks*) evtRecTrkCol->At(shower_id);
      ...
      if ((it.first->decayMode()==200) && (it.second->decayMode()==201)) {
         ...
         for (unsigned charged_id = 0; charged_id < signalD->tracks().size() ; ++charged_id) {
            ...
            if (is_pion_dtag( signalD->tracks()[charged_id], signalD)) {
               ...
               cerr << "unknown type of charged track! track_id: " << signalD->tracks()[charged_id] << endl;
            }

            DstEvtRecTracks * charged_track = (DstEvtRecTracks*) evtRecTrkCol->At(signalD->tracks()[charged_id]);
            HepLorentzVector charged4Momentum = charged_track->mdcKalTrack()->p4(charged_mass);
         ...
         {
            DstEvtRecTracks * photon_track = (DstEvtRecTracks*) evtRecTrkCol->At(signalD->showers()[photon_num]);

            Hep3Vector photonShowerVector = getShowerVector(photon_track->emcShower());
         ...
         HepLorentzVector missing4MomentumBC_cms = calc_miss_4momenum(evtRecDTag, observed4Momentum);
         u_miss = missing4MomentumBC_cms.e() - missing4MomentumBC_cms.vect().mag();
...
//-----------------------------------------------------------------------------
{
   if( selector->Verbose() ) cout << " DoubleDTagEndJob() " << endl;
   delete m_abscor;
}
