...
// TestPID                                                              //
//                                                                      //
// This is test example of user functions for ParticleID library        //
//                                                                      //
// The main differences from BOSS version:                              //
// 1) You MUST point on directory with data-files. In most cases it is: //
//    pid->set_path( selector->AbsPath("Analysis/ParticleID") );        //
//    (function AbsPath(file) returns "path_to_base_Bean_dir/" + file)  //
...
//    cout << " pid= " << pid << endl;
  ...
  //-------------------------------------------------------
   ...
   for(int i = 0; i < ncharge; i++){
     DstEvtRecTracks* dst_tracks = (DstEvtRecTracks*) evtRecTrkCol->At(i);
...
//        TMdcTrack *mdcTrk = dst_tracks->mdcTrack();
       ...
       pid->setRecTrack(dst_tracks);
       pid->usePidSys(pid->useTof1() | pid->useTof2() | pid->useDedx());
...
//        pid->usePidSys(pid->useTof1() | pid->useTof2());
       ...
       if(pid->IsPidInfoValid()){
         fprintf(fout,"track #%i chi2(electron, muon, pion, kaon, proton):",i);
         fprintf(fout," %15.10f %15.10f %15.10f %15.10f %15.10f\n",
                pid->chi(0),pid->chi(1),pid->chi(2),pid->chi(3),pid->chi(4));
